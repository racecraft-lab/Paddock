...
- **FR-007**: Persist only the Product Line scope slice with Zustand key `pd:active-workspace:v1`; validate or clear stale/malformed/wrong-tenant/unauthorized/real-facility ids after `/api/workspaces`.
...
- **FR-016..FR-017**: Outbound — emit `area:<projects.area_slug>` alongside `pd:*`/`priority:*` when project has non-NULL `area_slug`; otherwise no `area:*` label.
...
- **SPEC-007 FRs**: Persist task dispositions and task artifacts; provide publish/read/admin/health/API/UI surfaces; apply Paddock Secret Detector v1 redaction/rejection; integrate artifact evidence with dispatch/review handoff; preserve existing task behavior when feature flags are OFF.
