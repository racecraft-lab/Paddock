...
- **Next after SPEC-009F:** SPEC-013A is the next recommended setup target because it is the highest-priority unblocked run-state prerequisite. SPEC-010A is in progress on branch `010a-generic-product-line-seeder`; SPEC-011 remains a parallel option when file ownership stays disjoint because it touches the optional security adapter.
...
- **Next spec:** SPEC-013A (`run-state-spine`) remains the recommended next critical-path `$speckit-setup` target. SPEC-010A setup is in progress on branch `010a-generic-product-line-seeder`; SPEC-011 remains an optional parallel start only when file ownership stays disjoint.
...
- SPEC-010A is in progress on branch `010a-generic-product-line-seeder`; SPEC-011 may also start when file ownership stays disjoint.
- SPEC-012B waits for SPEC-010B so harness-gardening rules encode real two-product-line behavior.
- SPEC-009F is complete; run `$speckit-setup SPEC-013A` for the next critical-path setup, or `$speckit-setup SPEC-011` only when the operator intentionally chooses disjoint optional security-sidecar work.
