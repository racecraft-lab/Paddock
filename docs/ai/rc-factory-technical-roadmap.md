...
- **Next unblocked setup:** no active implementation workflow should be treated as in progress on `main` for SPEC-010A or SPEC-013A. SPEC-013A1 setup is in progress on `013a1-github-sync-automation`; SPEC-010B is also unblocked by SPEC-010A acceptance but still depends on Product Line B preflight cleanup. SPEC-011 remains a parallel option when file ownership stays disjoint because it touches the optional security adapter.
...
- **Control-plane path:** SPEC-013A is complete after post-merge UAT; SPEC-013A1 setup is in progress before claim/reconciliation and retry state proceed. These specs do not launch harnesses.
...
- **Next setup:** SPEC-013A1 setup is in progress on `013a1-github-sync-automation` after SPEC-013A UAT closeout. SPEC-010B is unblocked by SPEC-010A acceptance but should still complete Product Line B preflight cleanup before smoke. SPEC-011 remains an optional parallel start only when file ownership stays disjoint.
...
- SPEC-009F and SPEC-013A are complete after PR #58 and 2026-05-22 post-merge UAT; SPEC-013A1 setup is in progress.
