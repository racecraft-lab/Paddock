...
- **Current active setup:** SPEC-014A setup is active on branch `014a-sandbox-lifecycle-contract` for sandbox-lifecycle planning only. SPEC-013C remains the next control-plane retry/debug candidate; neither should duplicate SPEC-013B claim/reconciliation authority. SPEC-010B is also unblocked by SPEC-010A acceptance but still depends on Product Line B preflight cleanup. SPEC-011 remains a parallel option when file ownership stays disjoint because it touches the optional security adapter.
