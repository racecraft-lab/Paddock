...
4. **Dev-first, flag-scoped canary on live.** Write and commit changes in a development worktree. Promote via PR merge to `main` -> `git fetch` + `pnpm build` + restart the service in the live worktree. The "canary" is a feature flag flipped for ONE workspace (for example, the facility workspace or a dedicated test workspace) on the live service, validated, then promoted to wider workspaces. There is no separate canary environment on the operator node; there is ONE live MC and flag-scoping provides the safety.
...
| SPEC-008 | 7 | Resource Governance and Cost Tracker Enforcement | resource-governance | In Progress | P2 | SPEC-001, SPEC-002, SPEC-002A, SPEC-004 | SPEC-009 | Phase 7 |
