...
| SPEC-009C4 | 8C4 | Owner Merge Gate and Done Reconciliation | owner-merge-reconciliation | In Progress | P0 | SPEC-009C3 | SPEC-009D, SPEC-010B | Phase 8C4 |
...
**Current roadmap note:** SPEC-001, SPEC-002, SPEC-002A, SPEC-003, SPEC-004, SPEC-005, SPEC-006, SPEC-007, SPEC-008, SPEC-009A, SPEC-009B, SPEC-009C1, SPEC-009C2, and SPEC-009C3 are complete per the implementation evidence recorded below. Recent merge evidence includes SPEC-004 PR #22 as `20643d8`, SPEC-005 PR #23 as `851571f`, SPEC-006 PR #21 as `dbb6c75`, SPEC-007 PR #25 as `953f29b`, SPEC-008 PR #26 as `bd9a693`, SPEC-009A PR #28 as `2b78970e`, SPEC-009B PR #30 as `1d5c994c`, SPEC-009C1 PR #34 as `7d544f39`, the SPEC-009C1 post-merge routing fix PR #40 as `e6ee19ee`, SPEC-009C2 PR #43 as `a63afdea`, the SPEC-009C2 post-merge assignee fix PR #46 as `19b2db98`, and SPEC-009C3 PR #48 as `ac7760a2`. SPEC-009C4 implementation and live merge-gate UAT have passed on branch `009c4-owner-merge-reconciliation`; roadmap status remains `In Progress` until the implementation PR is merged and the target deployment promotion/UAT policy is satisfied.

- **Active after SPEC-009C3:** SPEC-009C4 remains the current self-hosting critical-path merge/reconciliation slice until its implementation PR is merged and promoted; SPEC-010A remains unblocked; SPEC-011 and SPEC-012A remain parallel options because they touch an optional security adapter and docs/process indexing respectively.
...
Phase 0 through Phase 8C3 are complete and remain the substrate for all later work. SPEC-009C4 is the active self-hosting merge/reconciliation spec with branch implementation and live merge-gate UAT complete, but it remains open until implementation PR merge and target deployment promotion/UAT evidence satisfy the status policy; SPEC-010A, SPEC-011, and SPEC-012A remain available parallel starts. The SPEC-009C family is the first practical self-hosting gate, split into ingest, triage handoff, remediation-to-owner, and merge reconciliation so each PR is reviewable. SPEC-010A extracts the reusable seeder from the Mission Control-specific path. SPEC-009D is the bridge from pilot smoke to formal run-state, SPEC-009E turns pilot eligibility/evidence into operator-visible read-only surfaces, and SPEC-009F owns production routing/evidence for non-remediation triage outcomes. SPEC-013A-C own run-state, GitHub sync automation, claim/reconciliation, and retry authority. SPEC-014A-D execute already-claimed work and must not own tracker truth, successor selection, governance, or auto-merge policy.

Parallel agents may work simultaneously only when they own disjoint primary files and state:

- SPEC-009C4 is active after SPEC-009C3 until its implementation PR is merged and promoted; SPEC-010A, SPEC-011, and SPEC-012A may also start because their primary files are disjoint from the current self-hosting merge/reconciliation slice.
- SPEC-010A may run while SPEC-009C4 is being executed or smoked when file ownership stays disjoint.
