# Paddock AI Software Factory — Technical Roadmap

> For SpecKit-Pro ingestion. Companion to the PRD at `docs/rc-factory-v1-prd.md`. Every phase is ship-safe on its own (additive migrations + feature flags), sequenced for Paddock's product direction, operator rollout safety, and reviewability.
...
4. **Dev-first, flag-scoped canary on live.** Write and commit changes in the PR worktree, merge through GitHub, then promote the merged `main` branch on the operator node. As of live verification on 2026-05-05, `paddock.service` runs from `<operator-home>/paddock` on `main`, with runtime data in `<operator-data-dir>`; `<retired-live-worktree>` is no longer present. Promotion is `git fetch`/update, `pnpm build`, and restart of `paddock.service`. The "canary" is a feature flag flipped for ONE workspace (e.g., the facility workspace or a dedicated test workspace) on the live service, validated, then promoted to wider workspaces. OpenClaw is a separate active service from `<openclaw-release-symlink>` and should be restarted only when gateway/runtime code or config changes.
5. **Install compatibility gate** on every PR: existing Paddock installs must keep running with defaults, flags OFF, additive schema, and documented rollback.
6. **Prefer additive, reversible extensions over destructive schema or runtime changes.** If the same goal can be achieved with an additive adapter, config path, or feature-flagged runtime hook, choose that before irreversible state changes.
7. **Operator-specific adapters remain optional.** They must be disabled by default and no-op cleanly when absent.
8. **Adopt Symphony as a contract, not a stack.** Paddock may borrow Symphony's language-agnostic control-plane pattern — tracker-driven dispatch, per-task workspaces, repo-owned workflow policy, bounded concurrency, retries, reconciliation, and observability — while keeping Next.js/React/TypeScript/SQLite, GitHub-first sync, and existing SpecKit task-chain governance.
9. **Keep harnesses pluggable and explicit.** Paddock owns tracker truth, claims, governance, artifacts, review packets, and run-state reconciliation. Codex/ChatGPT, Claude Code, OpenClaw, Hermes, OpenCode, or later systems execute through declared harness adapters that publish launch/resume/transcript/token/tool/MCP/memory/skill/plugin/sandbox capabilities and fail closed when a capability is absent.
10. **Separate tracker truth from local projections.** For SPEC-009 and later Symphony-aligned work, GitHub Issues are the v1 source work item and tracker of record. Paddock tasks store synchronized projections, chain state, assignments, artifacts, governance, run metadata, and views. Manual/local tasks remain supported for non-Symphony work, but they are not pilot runner work items without explicit GitHub linkage.
...
Paddock adapts these references to GitHub Issues, SpecKit workflow
...
4. **Parallel-agent ownership.** Specs may run in parallel only when their strict scopes do not edit the same primary files or mutate the same runtime state. The roadmap must name "can run with" and "blocked by" relationships so Paddock can assign multiple agents safely.
...
6. **Self-hosting bias.** Once SPEC-009D lands, every later spec should be represented as a GitHub issue ingested into Paddock, routed first through the Paddock Issue Triage workflow family, then assigned to Issue Remediation, SpecKit/SDD, specialist review, or closure from Paddock evidence where possible.
...
Every spec PR must produce a capability a human can review on the target Paddock deployment after merge. "Merged" means code/docs are on `main`; it does not mean the spec is operationally accepted.

1. **Deploy after merge.** Promote merged `main` to the target Paddock deployment, rebuild, restart the relevant service, and record the deployed commit before marking a spec operationally accepted. Process-only specs still require a fresh agent/operator to exercise the new repo workflow from the deployed or post-merge checkout.
...
4. **Open remediation as GitHub issues.** Any UAT defect becomes a GitHub issue with the affected spec id, expected behavior, observed behavior, reproduction steps, evidence links, and rollback state. Before the SPEC-009C family and SPEC-009D, this can be created manually. After the self-hosting pilot lands, Paddock should ingest and route these issues itself.
5. **Resolve through the factory loop.** UAT remediation issues are worked through Paddock when the required self-hosting capability exists: assign, claim, implement, review, merge by a human, redeploy, re-enable the same flags, and rerun UAT. The spec stays `UAT Remediation` or `Blocked` until the capability fully passes HITL UAT or a documented operator defer decision moves the issue to a later spec.
...
SPEC-013A-D and SPEC-014A-D must extend the existing Paddock control-plane seams instead of designing a parallel runner:
...
- The operator node deployment proves the desired separation: Paddock is the product/task/governance control plane, while OpenClaw gateway/node services provide an optional harness/runtime substrate. The same adapter contract must also support Paddock-owned worktrees/sandboxes when Codex, Claude Code, Hermes, OpenCode, or another harness is selected directly.
...
|---|---|---|---|---|
| 0 | Foundation migrations (M53–M61) | Yes | None — pure schema | — |
| 1 | Product-line switcher + `activeWorkspace` | Yes | `FEATURE_WORKSPACE_SWITCHER` | Phase 8B |
| 1A | Spec archive + evidence retention | Yes | None — process/tooling | Phase 2+ |
| 2 | Aegis refactor (facility singleton) | Yes (shim) | `FEATURE_GLOBAL_AEGIS` | Phase 3, 8C |
| 3 | Task pipeline engine + routing | Yes | `FEATURE_TASK_PIPELINES` | Phase 4, 6, 8A, 8C |
| 4 | `ready_for_owner` state + two-step terminal | Yes | `FEATURE_TWO_STEP_TERMINAL` | Phase 8C |
| 5 | Area-label GitHub sync | Yes | `FEATURE_AREA_LABEL_ROUTING` | Phase 8B, 8C |
| 6 | Disposition logging + artifact store + audit/admin panels | Yes | `FEATURE_DISPOSITION_LOGGING`, `FEATURE_TASK_ARTIFACTS` | Phase 8C, 8D |
| 7 | Resource governance + Cost Tracker enforcement | Yes | `FEATURE_RESOURCE_GOVERNANCE`, `FEATURE_OPENCLAW_HEALTH_COSTS` | Phase 8A, 8B, 8C |
| 7.5 | CrabTrap honeypot adapter | Yes | `FEATURE_CRABTRAP_HONEYPOT` | Parallel after Phase 7 |
| 8A | Workflow contract roundtrip | Yes | None — contract tooling | Phase 8B, Phase 10A |
| 8B | Paddock product-line seed + flag activation | Yes | `PILOT_PADDOCK_E2E` | Phase 8C, Phase 9A |
| 8C | GitHub-linked Paddock pilot smoke | Pilot gate | `PILOT_PADDOCK_E2E` | Phase 8D, Phase 9B |
| 8D | Pilot review packet + lifecycle snapshot | Yes | `PILOT_PADDOCK_E2E` | Phase 11A |
| 8E | Pilot eligibility + evidence surfaces | Yes | None — read-only task evidence surface | Phase 8F, Phase 11A |
| 8F | Production triage outcome routing | Yes | Pilot/product-line scope to be defined in setup | Later production triage lanes |
| 9A | Generic product-line seeder | Yes | None — seed tooling | Phase 9B |
| 9B | Product Line B onboarding smoke | Post-pilot | Disabled workspace until operator enablement | Phase 10B |
| 10A | Repo knowledge index + AGENTS map | Yes | None — process/tooling | Phase 10B, Phase 11A |
...
Paddock installs the setup-managed
...
project's current core templates so Paddock's existing UI and archive
...
Each spec should be executed from its generated worktree/branch. The mini-spec sections below are the canonical detailed source for scope, deliverables, acceptance criteria, rollback, rollout notes, and dependency/parallelization rules.
...
- **Pilot issue reproducibility:** SPEC-009C1 first tries an eligible open `racecraft-lab/Paddock` GitHub issue labeled `pd:inbox` and `priority:*`. If no safe live candidate exists, the operator smoke script creates a synthetic GitHub issue with title `[mc-pilot] synthetic e2e issue` and labels `pd:inbox priority:medium area:dev`. The pilot root task must be created by GitHub ingest/sync; local-only tasks created directly through `/api/tasks` or the task board do not satisfy the pilot source-of-truth gate.
...
| SPEC-009B | 8B | Paddock Product-Line Seed and Flag Activation | paddock-seed | Complete | P0 | SPEC-009A, SPEC-006, SPEC-008 | SPEC-009C1, SPEC-010A | Phase 8B |
...
| SPEC-010A | 9A | Generic Product-Line Seeder | generic-product-line-seeder | Complete | P2 | SPEC-002A, SPEC-009B | SPEC-010B | Phase 9A |
| SPEC-010B | 9B | Product Line B Onboarding Smoke | product-line-b-smoke | Complete | P2 | SPEC-009C4, SPEC-010A | SPEC-012B | Phase 9B |
| SPEC-011 | 7.5 | CrabTrap Honeypot Adapter | crabtrap-honeypot | In Progress | P2 | SPEC-008 | — | Phase 7.5 |
...
| SPEC-012B | 10B | Harness-Gardening Drift Guards | harness-gardening-guards | Complete | P1 | SPEC-010B, SPEC-012A | Later cleanup specs | Phase 10B |
...
| SPEC-014C | 12C | First Real Harness Adapter Pilot | first-real-harness-adapter | Complete | P1 | SPEC-013D, SPEC-014B | SPEC-014D, SPEC-014E, SPEC-014F, SPEC-015A | Phase 12C |
...
**Current roadmap note:** SPEC-001, SPEC-002, SPEC-002A, SPEC-003, SPEC-004, SPEC-005, SPEC-006, SPEC-007, SPEC-008, SPEC-009A, SPEC-009B, SPEC-009C1, SPEC-009C2, SPEC-009C3, SPEC-009C4, SPEC-009D, SPEC-009E, SPEC-009F, SPEC-010A, SPEC-010B, SPEC-012A, SPEC-012B, SPEC-013A, SPEC-013A1, SPEC-013B, SPEC-013C, SPEC-013D, SPEC-014A, SPEC-014B, and SPEC-014C are complete per the implementation and UAT evidence recorded below. Recent merge evidence includes SPEC-004 PR #22 as `20643d8`, SPEC-005 PR #23 as `851571f`, SPEC-006 PR #21 as `dbb6c75`, SPEC-007 PR #25 as `953f29b`, SPEC-008 PR #26 as `bd9a693`, SPEC-009A PR #28 as `2b78970e`, SPEC-009B PR #30 as `1d5c994c`, SPEC-009C1 PR #34 as `7d544f39`, the SPEC-009C1 post-merge routing fix PR #40 as `e6ee19ee`, SPEC-009C2 PR #43 as `a63afdea`, the SPEC-009C2 post-merge assignee fix PR #46 as `19b2db98`, SPEC-009C3 PR #48 as `ac7760a2`, SPEC-009C4 PR #52 as `ddc709f2`, SPEC-009D PR #54 as `765264b`, SPEC-009E PR #55 as `40507874`, SPEC-009F PR #57 as `d396ed2`, SPEC-012A PR #56 as `a5e3fbec`, SPEC-013A PR #58 as `a3a79250`, SPEC-010A PR #59 as `9be6b544`, SPEC-010B PR #83 as `d5308aa0`, SPEC-012B PR #84 as `77c6ce44`, SPEC-013A1 PR #60 as `41e9df68`, SPEC-013B PR #62 as `5e61d0ff`, SPEC-013C PR #63 as `42ff5ab7`, SPEC-014A PR #64 as `c01d9e44`, SPEC-013D PR #65 as `50bf05e5`, SPEC-014B PR #76 as `e7921a6f`, and SPEC-014C PR #79 as `0af176a5`. SPEC-010A and SPEC-013A post-merge UAT closeout was recorded on 2026-05-22 from merged `main` checkout `028abca9` with disposable local UAT databases, rebuilt standalone browser evidence, flag/process scope checks, no-mutation checks, and cleanup evidence. SPEC-013A1 post-merge HITL UAT was operator-confirmed complete on 2026-05-27 after PR #60 merged, covering automatic GitHub polling enablement, lifecycle/status visibility, disable behavior, manual sync fallback, and duplicate-ingestion safety. SPEC-013B post-merge target UAT completed on HAL on 2026-05-27 after PR #62 merged, covering deployment promotion to `5e61d0ffc02f9345b265cd5420660d02bf693016`, one claim/launch path, duplicate prevention, terminal release, governance blocking, negative-intake exclusion, read-model visibility, and zero disposable-row residue. SPEC-013C and SPEC-014A post-merge target UAT completed on HAL on May 29, 2026 CDT after PR #63 and PR #64 merged, covering deployment promotion to `c01d9e44ec826d94fa5916284c51453e5ec339ee`, claim-control release/cancel/retry/backoff/idempotency/stale/auth/flag-off cases, sandbox fake lifecycle owners `paddock`, `openclaw`, and `external_harness`, enabled and disabled read API evidence, and zero disposable-row residue. SPEC-013D merged on 2026-06-01 as PR #65 with local manual browser UAT, focused component and Playwright evidence, Docker-backed visual verification, Storybook/Playwright visual review evidence, and archived source provenance; HAL target UAT completed on 2026-06-02 CDT (`2026-06-03T01:08:15Z`) against live `main` commit `3ed79e26a19e6d78033ca0e13fdab01bb8aca01a`, covering retry/release/cancel, stale expected state, active backoff, backoff override, feature-flag-off, active service health, and zero disposable-row residue. SPEC-014B merged as PR #76 and completed HAL target UAT on 2026-06-04 CDT against live `main` commit `e7921a6f0e1e0a2a8042e9366be6a17beeb1e58b` with marker `SPEC-014B-HAL-UAT-20260604194737`, covering runtime-inventory auth/scope errors, assigned/unassigned fake registry states, eligible Paddock-owned lifecycle evidence, blocked external fake reasons, invalid capability rejection, feature-flag-off blocking, `/api/agents` compatibility, and zero disposable-row residue. SPEC-014C merged as PR #79 on 2026-06-05 as `0af176a5e5aebec11babed1ae034f18810b5f7e9` after HAL branch UAT marker `SPEC-014C-HAL-UAT-20260605121830`, covering one real Codex app-server stdio launch, descriptor-only evidence, deterministic failure fixtures, workspace-scoped flag-off blocking, no terminal/GitHub/successor/governance mutation, PR checks, and zero marker residue. SPEC-010B merged as PR #83 on 2026-06-06 as `d5308aa0723c48b670e88c6814b4e4a90892d74f` after local verification and HAL UAT marker `SPEC-010B-HAL-20260606040404`, covering disabled-by-default seed, explicit smoke enablement, synthetic Paddock issue evidence, Product Line A isolation, final Product Line B disablement, zero sync-owner/GitHub-sync/dispatch/smoke/side-effect counters, 47/47 verified tasks, and passed Quality Gate, CodeQL, and visual approval checks. SPEC-012B merged as PR #84 on 2026-06-07 as `77c6ce44bb10ac89b43ffcaab2b9ea35e7ea2f39`, covering offline harness-gardening drift guards, deterministic report/recommendation contracts, warning-only advisory signals, guardrails integration, static scope-control, 55/55 verified tasks, and no runtime product behavior or live mutation. SPEC-009F has local implementation verification, fixture UAT, merge evidence, and operator-confirmed target deployment/HITL UAT closeout recorded on 2026-05-22. SPEC-009C4 has target HAL deployment and UAT replay evidence recorded; SPEC-009D has packet UAT, merge, and main CI evidence recorded; SPEC-009E has task evidence surface UAT, merge, and main CI evidence recorded; SPEC-012A has repo knowledge index UAT, merge, visual approval, and main CI evidence recorded.
...
**Archive hygiene refresh (2026-06-05):** SPEC-014C provenance, recovery commands, merge evidence, HAL UAT evidence, and status ledgers are recorded in `.specify/memory/`, the workflow ledger, roadmap, AGENTS map, and autopilot state. Active source cleanup remains intentionally unapplied because no explicit `--apply-cleanup` run was requested.

**Archive hygiene refresh (2026-06-06):** SPEC-010B provenance, recovery commands, PR #83 merge evidence, HAL UAT evidence, and status ledgers are recorded in `.specify/memory/`, the workflow ledger, roadmap, AGENTS map, and autopilot state. Active source cleanup remains intentionally unapplied because no explicit `--apply-cleanup` run was requested.

**Archive hygiene refresh (2026-06-07):** SPEC-012B provenance, recovery commands, PR #84 merge evidence, guardrail evidence, and status ledgers are recorded in the workflow ledger, roadmap, AGENTS map, and autopilot state. Active source cleanup remains intentionally unapplied because no explicit safe-base `--apply-cleanup` run was requested.

- **Current active setup:** SPEC-011 is in progress on branch `011-crabtrap-honeypot` with scaffold artifacts in `docs/ai/specs/.process/` and `specs/011-crabtrap-honeypot/`. No SPEC-010B, SPEC-012B, SPEC-013C, SPEC-013D, SPEC-014A, SPEC-014B, or SPEC-014C implementation workflow remains active after PR #83/#84/#63/#65/#64/#76/#79 merge and recorded post-implementation evidence. SPEC-014D/E/F and SPEC-015A are pending follow-ons. SPEC-015A is the recorded IA/UI follow-up from SPEC-014C HAL browser UAT because workflow contracts/templates/pipelines need a first-class operator page instead of being stacked above Agent Squad on `/agents`.
...
- **Scale/doc parallel path:** SPEC-010A is complete after post-merge UAT; SPEC-010B is complete after PR #83 merge and HAL Product Line B smoke evidence; SPEC-012B is complete after PR #84 merge and now provides offline drift-guard recommendations without live mutation.
...
- **Runner path:** SPEC-014A, SPEC-014B, and SPEC-014C are complete after post-merge evidence. SPEC-014C proved the first real Codex app-server adapter on top of the manifest, fake registry, and runtime-inventory references with direct `codex app-server --listen stdio://` transport, descriptor-only run/attempt/activity persistence, focused cluster 8 files / 89 tests, scope guard, typecheck, lint, build, verify/verify-tasks/review/cleanup/retrospective reports, HAL deployed-branch service evidence, one real launch, deterministic failure fixtures through shared code paths, workspace-scoped flags, zero-residue cleanup proof, and PR #79 merge. SPEC-014D may run in parallel only if it owns disjoint adapter modules, fixtures, and deployment docs. SPEC-014E owns richer harness transcript/event retention, replay/debug export, quarantine, and opt-in raw capture policy after SPEC-014C proves the first real adapter. SPEC-014F owns live harness operator intervention UI for user-input/tool-approval prompts; SPEC-014C must fail closed on those events until SPEC-014F is implemented. SPEC-015A owns the separate workflow/orchestration page split and must not change harness adapter semantics, claim control, successor selection, governance, or auto-merge behavior.
...
| SPEC-009A | Paddock workflow policy is repo-owned and roundtrippable | Process-only contract import/export | Export/import the Paddock workflow family, inspect no-op parity hashes, and verify invalid contracts fail closed visibly |
| SPEC-009B | Paddock is seeded as Product Line A without launching work | `PILOT_PADDOCK_E2E` seed scope only | Run seed twice on target deployment, inspect workspace/departments/agents/repo/templates/flags/governance plus separate Issue Triage and Issue Remediation workflow families, and confirm no issue claim/dispatch |
| SPEC-009C1 | One GitHub issue enters the pilot as an eligible Paddock task | `PILOT_PADDOCK_E2E` for one real or synthetic issue | Ingest or create the pilot issue, prove GitHub is tracker-of-record, verify eligibility, and confirm local-only tasks cannot enter the pilot |
| SPEC-009C2 | Issue Triage hands actionable work to Issue Remediation planning | `PILOT_PADDOCK_E2E` for the pilot issue | Drive triage to `ACTIONABLE_REMEDIATION`, persist disposition/artifact evidence, and verify duplicate/OBE/invalid/needs-human/needs-specialist/`NEEDS_SPEC` outcomes do not enter remediation |
| SPEC-009C3 | The remediation chain reaches `ready_for_owner` | `PILOT_PADDOCK_E2E` for the pilot issue | Execute remediation planning, dev, review, and Aegis for the pilot issue until the linked PR-producing task reaches `ready_for_owner` |
| SPEC-009C4 | Human merge and GitHub sync reconcile the pilot to `done` | `PILOT_PADDOCK_E2E` for the pilot issue | Merge at `G_PILOT_MERGE`, sync GitHub state, and verify `ready_for_owner -> done` reconciliation without duplicate launch or local-only completion |
| SPEC-009D | Pilot work leaves a reviewable lifecycle packet | `PILOT_PADDOCK_E2E` review-packet scope | Inspect packet contents for issue/PR/artifacts/governance/Aegis/owner gate/current stage and explicitly deferred run/sandbox fields |
...
| SPEC-009F | Production triage outcomes route to the right non-remediation lanes | `PILOT_PADDOCK_E2E` product-line scope for v1 unless Clarify proves a dedicated flag is required | Drive `NEEDS_SPEC`, needs-human, needs-specialist, duplicate, obsolete, and invalid fixtures and inspect correct lane/evidence without a remediation successor |
| SPEC-010A | Product-line seeding is reusable beyond Paddock | Process-only seeder config | Recreate the Paddock seed from generic config in a safe target scope and verify incomplete/unsafe configs reject without mutation |
...
| SPEC-011 | CrabTrap can surface honeypot evidence without being required | `FEATURE_CRABTRAP_HONEYPOT` and valid/missing adapter config | Verify flag OFF and absent binary no-op, then replay one valid and one malformed signed fixture and inspect activities |
...
| SPEC-014A | Sandbox lifecycle is explicit, bounded, and flag-gated | `FEATURE_AGENT_RUNNER_SANDBOXES` with fake lifecycle | Create fake Paddock/OpenClaw/external lifecycles, verify bounded paths/handles/events/cleanup, and confirm flag OFF blocks create/run |
...
4. **Global flag without workspace context:** when called with no workspace (e.g., from `auto-route` cron loops), resolution uses `workspace_id = null` → returns OFF unless an env var explicitly forces a value (rare, used only for `PILOT_PADDOCK_E2E`).

Phase deliverables that name a flag (e.g., `FEATURE_WORKSPACE_SWITCHER`, `FEATURE_GLOBAL_AEGIS`, `FEATURE_TASK_PIPELINES`, `FEATURE_TWO_STEP_TERMINAL`, `FEATURE_AREA_LABEL_ROUTING`, `FEATURE_DISPOSITION_LOGGING`, `FEATURE_TASK_ARTIFACTS`, `FEATURE_RESOURCE_GOVERNANCE`, `FEATURE_OPENCLAW_HEALTH_COSTS`, `FEATURE_TASK_CONTROL_PLANE`, `FEATURE_AGENT_RUNNER_SANDBOXES`, `PILOT_PADDOCK_E2E`) MUST resolve through this helper. Inline `process.env.FEATURE_X` checks are forbidden; CI greps for them and fails on match.

`PILOT_PADDOCK_E2E` is the one exception that may also be flipped via env (it is operator-temporary). All other flags route through `workspaces.feature_flags`.
...
- **Autopilot notes:** Treat migrations as the only implementation surface. Verify live schema truth before assuming `agents.workspace_path` or a `tasks.status` CHECK constraint. Preserve null-default / flag-off compatibility and document rollback impact before implementation.
...
- **Status:** Complete
- **Branch status:** PR #25 (https://github.com/racecraft-lab/Paddock/pull/25) merged to `main` on 2026-05-02 as `953f29b`.
- **Priority:** P2
...
- **Status:** Complete
- **Branch status:** PR #26 (https://github.com/racecraft-lab/Paddock/pull/26) merged to `main` on 2026-05-04 as `bd9a693`.
- **Priority:** P2
...
- **Autopilot notes:** Do not duplicate token/cost telemetry. `FEATURE_RESOURCE_GOVERNANCE=false` preserves legacy scheduler behavior. `FEATURE_OPENCLAW_HEALTH_COSTS` is operator-specific optional, runtime-only, absent-safe, and must require no v1 schema migration.
...
- **Definition of done:** Contract import/export parity passes for the Paddock workflow family; invalid contract fixtures fail closed with operator-visible errors; existing `workflow_templates` behavior is unchanged unless the spec command explicitly imports the contract.
- **Implementation evidence:** Local implementation and post-implementation verification completed on branch `009a-workflow-contract-roundtrip` on 2026-05-06. Evidence includes exact direct `yaml@2.8.2`, canonical contract at `docs/ai/workflows/paddock/workflow-contract.yaml`, `pnpm workflow-contract` import/apply/export/recover tooling, generated Markdown review export with stable hash `workflow-contract-hash-v1:sha256:2f0e9ef6e21ca80039c49bc6398bf8f7bd1493be454ff5d7e381391b4b8884da`, additive M71 diagnostics/snapshot storage plus rollback SQL, read-only diagnostics API/UI, OpenAPI/API-index parity, fail-closed validation fixtures, full Vitest/build/typecheck/lint/focused Playwright evidence, final GitNexus embeddings rebuild copied to the primary checkout root, and guardrail verification confirming no pilot seed, dispatch, scheduler, runner, harness, GitHub sync, sandbox lifecycle, or governance evaluator path.

### SPEC-009B: Paddock Product-Line Seed and Flag Activation

- **Status:** Complete
- **Priority:** P0
- **Branch short name:** `paddock-seed`
- **Dependencies:** SPEC-009A, SPEC-006, SPEC-008
- **Enables:** SPEC-009C1, SPEC-010A
- **Scope source:** Phase 8B - Paddock product-line seed and flag activation
- **Acceptance criteria source:** Phase 8B Acceptance Criteria
- **Scope summary:** Seed Paddock as Product Line A with workspace, departments, agent assignments, GitHub repo routing, separate Issue Triage and Issue Remediation workflow families from the SPEC-009A contract mechanism, Phase 1-7 feature flags, and conservative governance policies. This proves configuration and policy shape without dispatching a live autonomous issue.
- **Tool count / tool names:** N/A - not a tool-surface spec
- **Strict Scope:** `scripts/seed-paddock-product-line.ts` if authored in TypeScript; seed fixtures/config docs; focused tests for idempotent seed output, GitHub repo preservation, assignments, feature flags, and governance rows.
- **Autopilot notes:** Preserve GitHub linkage and sync metadata for previously synced Paddock issues. The app remains the setup/observability/control surface; it is not the pilot intake path.
- **Definition of done:** Running the seed twice leaves one Paddock product-line workspace with expected departments, assignments, repo config, imported Issue Triage and Issue Remediation templates, feature flags, and governance policies; no autonomous issue is claimed or dispatched by this spec.
- **Implementation evidence:** Local implementation and post-implementation verification completed on branch `009b-paddock-seed` on 2026-05-07. Evidence includes the Paddock-specific seed/preflight/verify CLI, idempotent clean-target seed evidence, non-destructive blocked-preflight evidence for non-Paddock residue, backup/export-first FocusEngine/OpenClaw cleanup runbook, canonical `PILOT_PADDOCK_E2E` registry and runbook alignment, corrected Paddock workflow-contract slugs plus regenerated Markdown export hash `workflow-contract-hash-v1:sha256:4e485c97c7136a79619c362ba7de26cd9439ea49f60ea54a2f14414a7a287c92`, conservative advisory governance rows, focused Vitest coverage, typecheck/lint/build/e2e evidence, and guardrails confirming no synthetic issue, claim, dispatch, scheduler launch, runner state, sandbox lifecycle, generic Product Line B seeder, or auto-merge path.
...
- **Scope summary:** Select one eligible live `racecraft-lab/Paddock` issue or create one synthetic `[mc-pilot]` GitHub issue only when no safe live issue exists. Ingest/sync it into Paddock as a GitHub-linked pilot root task and prove local-only tasks cannot enter the pilot lane.
- **Tool count / tool names:** N/A - not a tool-surface spec
...
- **Tool count / tool names:** N/A - not a tool-surface spec
- **Strict Scope:** Paddock workflow-contract correction for triage disposition schema/routing, triage routing fixtures, disposition/artifact evidence, successor creation through existing task-chain helpers, and negative cases for duplicate, OBE, invalid, needs-human, needs-specialist, and `NEEDS_SPEC`.
...
- **Scope summary:** Record the intentional `G_PILOT_MERGE` human gate, merge the linked pilot PR, sync GitHub state back into Paddock, and prove `ready_for_owner -> done` reconciliation without duplicate launch or local-only terminal completion.
- **Tool count / tool names:** N/A - not a tool-surface spec
...
- **Implementation evidence:** Branch `009d-pilot-review-lifecycle` completed all 42 generated tasks and merged implementation PR #54 to `main` as `765264be667bd31d6266f606602a219312f72f23`. Implementation adds a stored-evidence-only packet derivation module, JSON/Markdown artifact publication through existing task artifact behavior, packet-local evidence states, SPEC-013/SPEC-014 deferrals, local-only/partial-proof exclusion, and strict TypeScript/ESLint coverage for SPEC-009D-owned files. Verification passed under Node 22.22.2: focused packet/artifact/disposition tests passed 20 tests, existing task-artifact seam tests passed 39 tests, `pnpm typecheck`, `pnpm lint`, `pnpm build`, `pnpm guardrails`, `pnpm audit:high`, `pnpm test` with 278 passed files / 2920 passed tests, and `pnpm test:e2e` with 646 passing Playwright tests. UAT on 2026-05-20 seeded a disposable Paddock database from retained issue #50 / PR #51 evidence, generated a `proven` packet at stage `done`, published JSON artifact `2` and Markdown artifact `3` through the real artifact store, inspected both through existing `/api/task-artifacts` routes, and cleaned seeded rows after backup. Post-merge `main` CI/CD checks for the merge commit passed: Quality Gate, CodeQL, Paddock UI E2E, Visual Storybook Snapshots, Playwright visual approval, and Storybook visual approval. No migration, new runtime dependency, packet-specific route, dashboard, fresh GitHub call, poller, claim authority, retry controls, sandbox lifecycle, adapter registry, or real harness execution was added.
...
- **Setup evidence:** Branch `009f-production-triage-routing` was created from `main` on 2026-05-21. Grill Me setup decisions require recommendation-only routing/evidence, typed lane artifacts in existing disposition/artifact/activity storage, task Evidence `triageRouting` display, `PILOT_PADDOCK_E2E` v1 scope, terminal Issue Triage outcomes without successors, and no automatic GitHub mutation, SpecKit setup, claim, runner, sandbox, adapter, or auto-merge behavior.
- **Implementation evidence:** PR #57 merged to `main` as `d396ed205b281d10a2b5cb95542209e816ebd95a` on 2026-05-22 after branch verification captured at `f9a18575` and post-implementation docs hygiene at `c8ef2ea1`. Local verification passed under Node 22.22.2: focused routing/dispatch tests (41), broader focused SPEC-009F tests (84), `pnpm api:parity`, `node scripts/spec-009f/check-scope-guards.mjs`, focused e2e UAT, `pnpm build`, `pnpm typecheck`, `pnpm lint`, full `pnpm test` (2,991 passed, 3 skipped, 84 todo), and full `pnpm test:e2e` (648 passed). GitHub merge checks passed for CodeQL, Quality Gate, and visual approval contexts. Reviewability diff gate passed under the approved exception because the behavior remains terminal triage-routing evidence plus the existing task Evidence extension, with the extra footprint in generated SpecKit artifacts, API parity, security payload tests, UAT fixtures, and guard tooling. Operator closeout on 2026-05-22 confirmed target deployment promotion, `PILOT_PADDOCK_E2E` target-scope validation, and HITL replay of the six non-remediation outcomes with no reported UAT defects. No migration, runtime dependency, live GitHub mutation, successor template, claim/runner/sandbox/adapter path, or auto-merge behavior was added.
- **Next setup:** SPEC-013B, SPEC-013C, SPEC-013D, SPEC-014A, SPEC-014B, SPEC-014C, SPEC-010B, and SPEC-012B are complete after merge and recorded evidence. SPEC-011 is now active on branch `011-crabtrap-honeypot`. SPEC-014D, SPEC-014E, SPEC-014F, and SPEC-015A remain available follow-on lanes when file ownership stays disjoint.
...
- **Scope summary:** Parameterize the Paddock seed path into a reusable product-line seeder that accepts product-line slug, display name, agent prefix, GitHub repo, workflow family, feature flags, and governance defaults. It does not onboard Product Line B or run a second smoke.
- **Tool count / tool names:** N/A - not a tool-surface spec
...
- **Post-merge UAT evidence:** On 2026-05-22, merged `main` checkout `028abca9` was tested under Node v22.22.2 with disposable DB `/private/tmp/paddock-uat-010a-013a.joRGvX/spec-010a-uat-r2.db` copied from the local `.data` baseline; local `.data/paddock.db` was not mutated. Generic `seed:product-line` preflight, apply, existing-target refusal, apply `--allow-existing`, verify, and `seed:paddock` wrapper verify all returned the expected structured statuses and stable snapshot hashes. Invalid reserved-flag config failed before writes with `FEATURE_FLAG_RESERVED_FUTURE_ENABLED`, `FEATURE_FLAG_CONFLICT`, and `CONFIG_CONFLICTING_DECLARATION`; no-mutation comparison passed and raw secret emission stayed false. Direct DB inspection confirmed Paddock seed shape with one workspace, six department projects, six assignments, nine workflow templates, three governance defaults, zero Product Line B workspaces, zero tasks, and zero runs. Static scope search found only negative safety-policy/evidence/test references and no Product Line B onboarding, GitHub mutation, dispatch, runner, sandbox, or SpecKit invocation drift.

### SPEC-010B: Product Line B Onboarding Smoke

- **Status:** Complete - PR #83 merged to `main` as `d5308aa0723c48b670e88c6814b4e4a90892d74f`; archive/status hygiene recorded
- **Priority:** P2
...
- **Scope summary:** Onboard Product Line B as the second product line, provision or register isolated agents through the configured harness substrate, configure its canonical repo, and run one live or synthetic issue through the already-proven pilot subset. Paddock Product Line A must remain unaffected.
- **Tool count / tool names:** N/A - not a tool-surface spec
...
- **Autopilot notes:** Manual "<1 operator-hour" timing is checklist-only. FocusEngine Paddock project/repo sync unlink, stale issue cleanup, and GitHub sync/triage cron cleanup are a Product Line B preflight prerequisite, not part of this spec. Existing OpenClaw runtime agents may remain as reusable runtime identities unless separately decommissioned, but Product Line B can use them only through explicit product/project role assignment after the runtime profile has been generalized and adapter eligibility passes. Docker, OpenClaw-owned, Paddock-owned worktree, or external-harness sandboxes remain valid choices according to the product-line config.
- **Setup evidence:** Branch `010b-product-line-b-smoke` was created from `origin/main` on 2026-06-05 with Grill Me setup decisions recorded in `docs/ai/specs/SPEC-010B-design-concept.md` and autopilot prompts in `docs/ai/specs/SPEC-010B-workflow.md`. HAL preflight against `/home/fredrick-gabelmann/paddock-data/paddock.db` passed with `status: ready`, `mutation_status: not_mutated`, empty residue, and matching before/after snapshot hashes. The setup path chooses generic `product-line-b`, synthetic `racecraft-lab/Paddock` smoke metadata, Paddock-owned fake/harness agents, disabled-by-default lifecycle, and no required live GitHub write or FocusEngine/OpenClaw identity reuse.
...
- **Implementation evidence:** PR #83 merged to `main` as `d5308aa0723c48b670e88c6814b4e4a90892d74f` on 2026-06-06T11:13:31Z. Evidence includes reviewed `product-line-b.yaml`, no-mutation preflight/apply/verify behavior, disabled-by-default workspace state, explicit smoke enablement, `spec-010b.synthetic_issue.v1` and `spec-010b.smoke_evidence.v1` proof, Product Line A scoped hash parity, zero sync-owner/GitHub-sync/dispatch/smoke residue after disablement, optional live GitHub non-requirement, and SPEC-014C adapter/runtime-inventory file-disjointness. Local final verification passed `direnv exec . pnpm build`, `direnv exec . pnpm typecheck`, `direnv exec . pnpm lint`, `direnv exec . pnpm test` with 361 files / 3,391 tests, `direnv exec . pnpm test:e2e` with 653 passed / 1 skipped, guardrails, strict TypeScript, and focused Product Line B tests. HAL UAT run id `SPEC-010B-HAL-20260606040404` seeded workspace `31`, kept Product Line B disabled except for the explicit smoke window, proved final disabled state with 6 Product Line B projects, 0 sync-enabled projects, 0 repo-sync-owner projects, 0 dispatch-eligible tasks, and Product Line A snapshot parity. PR checks passed for quality-gate, CodeQL, Playwright visual approval, and Storybook visual approval.
...
- **Implementation evidence:** Branch `012a-repo-knowledge-index` added `docs/ai/repo-knowledge-index.json`, `docs/ai/repo-knowledge-index.schema.json`, a concise root Repo Knowledge Map, fixture-backed validation, a fresh-agent proxy smoke script, focused package scripts, and `repo-knowledge-index` guardrail coverage through the existing Quality Gate `pnpm guardrails` path. Local verification on 2026-05-21 passed `pnpm knowledge:index:check`, `pnpm knowledge:index:smoke`, documented negative/warning fixture behavior through `pnpm knowledge:index:check -- --fixture ... --json`, `pnpm guardrails -- --suite repo-knowledge-index`, full `pnpm guardrails`, `pnpm typecheck`, `pnpm lint`, `pnpm build`, full `pnpm test`, `pnpm test:e2e`, and `git diff --check` under `direnv exec .` / Node v22.22.2 where required. UAT caught and fixed the guard parser path for pnpm's literal `--` separator before `--fixture`. PR #56 merged to `main` as `a5e3fbece82fddec548b70763a703893ba409813` on 2026-05-21; CodeQL, `quality-gate`, `docker-ui-e2e / visual-review-report`, `visual-storybook / visual-review-report`, `visual-review-approval/playwright`, and `visual-review-approval/storybook` passed. No runtime source behavior, migration, UI, scheduler/runner behavior, GitHub sync automation, sandbox lifecycle, harness adapter, generated `.gitnexus/` artifact, broad docs rewrite, or nested `AGENTS.md` rollout was added.

### SPEC-012B: Harness-Gardening Drift Guards

- **Status:** Complete - PR #84 merged to `main` as `77c6ce44bb10ac89b43ffcaab2b9ea35e7ea2f39`
- **Priority:** P1
...
- **Implementation evidence:** PR #84 merged on 2026-06-07T16:18:07Z. Evidence includes deterministic offline drift detectors, non-mutating Paddock cleanup-task and GitHub issue draft recommendations, report schema validation, warning-only freshness/cleanup signals, archive cleanup recommendation gating, guardrails suite wiring, static scope-control self-test/current-diff evidence, 55/55 verified tasks, verify/review/cleanup/retrospective reports, focused SPEC-012B tests, full guardrails, knowledge-index check, typecheck, lint, and unsandboxed full unit suite. No runtime product behavior, migration, UI/API endpoint, live GitHub/Paddock mutation, auto-merge, or automatic `specs/**` cleanup was added.
...
- **Post-merge UAT evidence:** On 2026-05-22, merged `main` checkout `028abca9` was tested under Node v22.22.2 with `FEATURE_TASK_CONTROL_PLANE` scoped to the SPEC-013A debug/read path only. Focused Vitest passed 4 files / 31 tests; `scripts/spec-013a/check-run-state-scope-guards.mjs` passed across 48 changed files; `pnpm build` rebuilt the standalone app; and focused Playwright `tests/e2e/spec-013a-task-stage-attempts.spec.ts` passed 1/1 with screenshot `test-results/spec-013a-task-stage-attempts/spec-013a-task-stage-attempts.png`. `node scripts/check-guardrails.mjs --suite task-pipeline` passed, proving legacy pipeline runtime remains table-blind. Disposable e2e DB inspection retained M76 marker `076_task_stage_attempts`, required attempt/event tables and indexes, `PRAGMA foreign_key_check` returned zero rows, and cleanup left zero attempts, zero events, and zero fixture tasks.
...
- **Implementation evidence:** Branch `013a1-github-sync-automation` completed SPEC-013A1 implementation and local UAT on 2026-05-23. Evidence includes Doctor/Verify/Verify Tasks/Review/Cleanup extension passes, remediation of lifecycle lease and manual run-id review blockers, focused review-remediation regression (4 files, 53 tests), `pnpm check:strict-scope`, and full `direnv exec . pnpm test:all` passing strict scope, lint, typecheck, 300 Vitest files, production build, and 651 Playwright tests. PR #60 merged to `main` as `41e9df68afb7bbb2b536dd5e9a4ed04f7ead47ca` on 2026-05-23 after CodeQL, quality gate, Docker UI visual report, Storybook visual report, Playwright visual report, and visual approval statuses passed. Follow-up PR #61 merged as `5c15f02f368a2f8db01adf64936c96f3c3173873` to align Codex integration with Claude. The retrospective records 100% task completion, 100% spec adherence, 0 critical findings, and `uat_blocked=false`.
- **Post-merge UAT evidence:** Operator-confirmed HITL UAT complete on 2026-05-27 for the SPEC-013A1 required gate: automatic GitHub issue polling can be enabled for one Product Line, lifecycle status/last-run/error state is inspectable, polling can be disabled, manual sync still works, and shared-repository owner filtering prevents duplicate ingestion. No UAT remediation issue is recorded.
...
- **Implementation evidence:** Branch `013c-retry-debug-surfaces` implemented M79 operator release reasons plus scoped idempotency replay storage, `POST /api/tasks/[id]/claim-control`, the optional `claim_control` read-model extension, bounded audit evidence, OpenAPI/API index updates, UAT scaffolding, and PR review packet evidence. Local verification passed under Node 22.22.2: focused SPEC-013C cluster 6 files / 39 tests, `pnpm typecheck`, `pnpm lint`, `pnpm api:parity`, `pnpm check:strict-scope`, full `pnpm test` outside sandbox with 308 files / 3190 tests, and `pnpm build` outside sandbox. PR #63 merged to `main` as `42ff5ab7ba7c35c9e7b80fbe652feb1dfedffb89` on 2026-05-30T01:33:03Z. HAL target deployment was promoted to `c01d9e44ec826d94fa5916284c51453e5ec339ee` on May 29, 2026 CDT after SPEC-014A also merged; `pnpm install --frozen-lockfile` was a lockfile no-op, `pnpm build` passed, `paddock.service` restarted and logged database migrations applied, `/login` returned HTTP 200, and `openclaw-gateway.service` remained active. Post-merge UAT replay `spec013c-014a-uat-1780110032087` verified active release, cancel, retry, failed retry, backoff block, backoff override, idempotency replay, body mismatch, stale state, unauthenticated and viewer rejection, feature-flag-off rejection, read-model before/after evidence, M79/M80 migration markers, and zero disposable-row residue. SPEC-013C remains API/debug authority only; SPEC-013D remains the operator UX adoption gate.
...
- **Autopilot notes:** Treat SPEC-013D as the UX completion gate for SPEC-013C, not as a place to re-open API semantics. Use existing Paddock task evidence patterns, keep controls hidden or disabled when `FEATURE_TASK_CONTROL_PLANE` is off or the task/stage is ineligible, and surface audit/debug outcomes returned by SPEC-013C rather than recomputing claim state in the client.
...
- **Implementation evidence:** Branch `013d-claim-control-operator-ux` added the task-detail `Claim control` section, closed operator-facing copy maps, existing-route client integration for the SPEC-013C `claim_control` read/mutation envelopes, inline confirmation and reason/override flows, same-submission idempotency retry handling, bounded success/error receipts, evidence/stage-attempt/task-list refreshes, Storybook review states, and the serial route-backed Playwright fixture with cleanup proof. The implementation deliberately added no migration, backend retry/release/cancel semantics, scheduler launch, dashboard, sandbox lifecycle, adapter registry, direct GitHub mutation, successor selection, or harness execution. Verification recorded focused component tests, `pnpm typecheck`, `pnpm lint`, `pnpm build`, full `pnpm test`, full `pnpm test:e2e`, local manual browser UAT, Docker-backed Playwright, and Storybook/Playwright visual review evidence in `docs/ai/specs/SPEC-013D-workflow.md` and `specs/013d-claim-control-operator-ux/uat-runbook.md`. PR #65 merged to `main` as `50bf05e573f15b5aab5e53367444bef1d0b7baaf` on 2026-06-01T14:27:30Z. Post-merge archive/status hygiene was recorded on 2026-06-01. HAL target UAT completed on 2026-06-02 CDT (`2026-06-03T01:08:15Z`) against running Paddock service commit `3ed79e26a19e6d78033ca0e13fdab01bb8aca01a` with marker `SPEC-013D-HAL-UAT-20260603010815`: retry returned `retry_ready`, release returned `released`, cancel returned `cancelled`, stale expected state returned `stale_state`, active backoff returned `retry_backoff_active`, backoff override returned `retry_ready` with `override_applied=true`, flag-off returned `flag_off`, `paddock.service` and `openclaw-gateway.service` stayed active, and cleanup counts were zero across disposable workspace, user/session, task, attempt, claim, idempotency, lifecycle-control, and activity rows. HAL could not fast-forward before UAT because `git fetch origin` failed with `Could not resolve host: github.com`; live commit already contained PR #65, so no promotion was attempted.
...
- **Scope summary:** Define deterministic, sanitized, product-line-scoped sandbox keys/paths and lifecycle hooks for `paddock`, `openclaw`, and `external_harness` ownership using fakes only. No real harness launches.
- **Tool count / tool names:** N/A - not a tool-surface spec
- **Strict Scope:** sandbox path/key helpers, lifecycle hook interface, fake owner implementations, cleanup policy, minimal read API, and tests for path traversal/rollback. No operator UI, no adapter manifest, no token accounting, no real runner.
- **Autopilot notes:** Sandbox ownership is a user/workflow choice. Paddock-owned worktrees, OpenClaw-owned sandboxes, and external harness handles must all fit the same lifecycle vocabulary. Before Specify or Plan, fetch the current OpenAI Harness Engineering article and Symphony announcement/SPEC into context and record how workspace safety, sandbox lifecycle, and context-legibility lessons map to Paddock.
...
- **Implementation evidence:** Branch `014a-sandbox-lifecycle-contract` implemented additive M80 lifecycle/event persistence, `FEATURE_AGENT_RUNNER_SANDBOXES`, bounded sandbox key/path helpers, production fake owners for `paddock`, `openclaw`, and `external_harness`, cleanup/rollback evidence, `sandbox_lifecycle.v1` read model and API route, OpenAPI/API index updates, scope guards, and rollback documentation. Local verification passed with focused SPEC-014A Vitest (34 tests), feature-flag/migration guard tests (44 tests), `pnpm typecheck`, `pnpm lint`, `pnpm api:parity`, `pnpm build`, and full `pnpm test` outside sandbox with 307 passed files / 3201 passed tests. PR #64 merged to `main` as `c01d9e44ec826d94fa5916284c51453e5ec339ee` on 2026-05-30T02:08:31Z. HAL target deployment was promoted to that commit on May 29, 2026 CDT; `pnpm install --frozen-lockfile` was a lockfile no-op, `pnpm build` passed, `paddock.service` restarted and logged database migrations applied, `/login` returned HTTP 200, and `openclaw-gateway.service` remained active. Post-merge UAT replay `spec013c-014a-uat-1780110032087` verified flag-off create block before enable, fake lifecycle completion for all three owners with durable `cleaned_up` status, enabled read API with 3 lifecycle rows and no unsafe host path/token payload, flag-off create block after disable, disabled read evidence preserving the lifecycle rows, M79/M80 migration markers, and zero disposable-row residue. SPEC-014A remains lifecycle contract only; SPEC-014B owns first runtime-inventory integration and fake adapter registry.
...
- **Operator UI ownership:** SPEC-014B owns the first operator-visible runtime-inventory integration for read-only sandbox lifecycle references produced by SPEC-014A. Richer lifecycle controls remain out of SPEC-014B unless a later SPEC-014C/D or dedicated UI/control spec ratifies them.
- **Tool count / tool names:** N/A - not a tool-surface spec
...
- **Autopilot notes:** A real adapter cannot land before the fake registry proves Paddock state is not Codex-specific. Visibility in runtime inventory is not eligibility for work; eligibility requires explicit project-role assignment, selected adapter capability support, product-line runner flag enablement, governance allow, and tracker-linked task eligibility. Before Specify or Plan, fetch the current external context and map Symphony SPEC client/workspace/validation requirements into the fake manifest contract.
...
- **Implementation evidence:** PR #76 merged to `main` as `e7921a6f0e1e0a2a8042e9366be6a17beeb1e58b`. HAL target deployment fast-forwarded live `main` to that commit, `pnpm install --frozen-lockfile` and `pnpm build` passed, `paddock.service` restarted successfully, `/login` and authenticated `/api/status` returned HTTP 200, and `openclaw-gateway.service` remained active. Post-merge target UAT marker `SPEC-014B-HAL-UAT-20260604194737` verified runtime-inventory auth/scope errors, fake registry assigned/unassigned states, eligible Paddock-owned lifecycle evidence, blocked external fake reasons, invalid capability rejection, feature-flag-off blocking, `/api/agents` compatibility, no read-model mutation, and zero disposable-row residue.
...
- **Scope summary:** Add the operator-specific optional adapter boundary for OpenClaw-owned sandboxes and external harness handles. OpenClaw may provide gateway/session messaging, sandbox preparation, process control, plugin harness selection, MCP/skills exposure, memory injection, optional health/cost telemetry, and imported runtime-agent inventory; Paddock still owns tracker state, project-role assignment, claims, governance, reconciliation, review packets, and handoff artifacts.
- **Tool count / tool names:** N/A - not a tool-surface spec
...
- [P0-AC13] The `ready_for_owner` safety gate makes no DB-level CHECK change and no application-level status-vocabulary change; ripgrep over the SPEC-001 diff finds zero occurrences of `CHECK (status`, `ready_for_owner`, or `pd:ready-for-owner` outside docs and rollback commentary.
...
- **Zustand store and transition API**: `activeWorkspace: Workspace | null` plus `setActiveProductLine(productLine | null, options)`.
  ...
  - **Implementation contract:** persist only the Product Line scope slice with `zustand/middleware` (storage = `localStorage`, key = `pd:active-workspace:v1`). Validate persisted scope after `/api/workspaces` before rendering mode-sensitive cached data.
  - **Cross-tab contract:** use `BroadcastChannel('pd:active-workspace')` messages shaped as `{ tenantId, userId/sessionId, productLineId|null, version, originTabId }`; ignore mismatched tenant/session, self echoes, and stale versions. Fall back gracefully when `BroadcastChannel` is unavailable.
...
- `src/store/index.ts` (add `activeWorkspace` slice + `persist` middleware + `BroadcastChannel` listener; the live store path is `src/store/index.ts`, not `src/store/paddock-store.ts`)
...
- [P1-AC5] Agent squad panel renders hierarchical tree: Facility (globals) -> Paddock -> {QA, Dev, ...} -> {agents}; duplicate global/local names do not merge stats and mutations use ids where ambiguity exists.
...
- [P1-AC11] `src/store/index.ts` exports Product Line scope persistence (key `pd:active-workspace:v1`, storage `localStorage`) and BroadcastChannel sync only for the new scope slice. Vitest unit-tests serialization, hydration validation, guarded broadcast handling, and fallback behavior.
...
- **Archive extension decision**: validate `spec-kit-archive` against the current SpecKit tooling and document whether Paddock installs, vendors, forks, or rejects it. Any adoption must pin a tag or commit and preserve MIT license metadata.
...
- **Schema validation library**: `src/lib/output-schema-validator.ts` — server-side JSON Schema validator using `ajv` as an explicit direct pinned dependency. Do not import transitive `ajv`. Configure a constrained Paddock schema profile with the following **numeric bounds**:
...
- **GitHub label**: `github-label-map.ts` — `STATUS_LABEL_MAP.ready_for_owner = 'pd:ready-for-owner'`; `ALL_STATUS_LABEL_NAMES` updated. `initializeLabels` auto-creates the label.
...
- [P4-AC6] `pd:ready-for-owner` label appears on linked GitHub issue when a Paddock task enters that state.
...
- **Outbound sync**: `pushTaskToGitHub` emits `area:<project_slug>` label alongside `pd:*` and `priority:*`.
...
Log every triage disposition to `task_dispositions` (D9). Add the shared Paddock task artifact store (D11) as the durable handoff plane between private agent sandboxes. Extend operator surfaces with disposition views and artifact admin/health controls.
...
- [P7-AC4] Project/status WIP policy prevents more than the configured number of Paddock Development tasks from entering `in_progress`.
...
Flip `FEATURE_RESOURCE_GOVERNANCE` OFF. Scheduler returns to legacy behavior. Tables and events remain for auditability. Existing Cost Tracker views continue to work. If needed, also flip `FEATURE_OPENCLAW_HEALTH_COSTS` OFF to remove the optional OpenClaw infra adapter without affecting governance core.
...
### Lane A - Self-Hosting Paddock Product Line

This is the critical path for using Paddock to finish its own roadmap. It onboards Paddock as the first product line, configures its agents/governance/GitHub integration, and proves the system by having agents work Paddock GitHub issues toward completion and deployment.
...
| Spec | Slice | Blocked By | Can Run With | Human Validation |
|---|---|---|---|---|
| SPEC-010A | Generic product-line seeder | SPEC-009B | SPEC-009C1/C2 | Reproduce Paddock seed from config without dispatching work |
| SPEC-010B | Product Line B smoke | SPEC-009C4, SPEC-010A, clean HAL Product Line B preflight evidence | SPEC-014C only with disjoint seed/config vs adapter files | Onboard Product Line B in under one operator-hour, run first issue smoke, disable workspace cleanly |
...
| Spec | Slice | Blocked By | Can Run With | Human Validation |
|---|---|---|---|---|
| SPEC-014A | Sandbox ownership + lifecycle contract | SPEC-013B | SPEC-013C | Create fake Paddock/OpenClaw/external sandbox lifecycles and confirm paths/handles/cleanup are bounded |
...
Phase 0 through Phase 8F, Phase 9A-9B, Phase 10A-10B, Phase 11A-11D, and Phase 12A-12C are complete and remain the substrate for all later work. SPEC-014C proved the first real Codex app-server adapter after SPEC-014B post-merge acceptance while preserving SPEC-013D's operator UX boundary plus SPEC-014B's manifest/runtime-inventory boundary; PR #79 merged as `0af176a5e5aebec11babed1ae034f18810b5f7e9` and HAL UAT passed on branch target `43989ac856696abb2ea764fed409da268b87c9a8`. SPEC-010B proved Product Line B as a second disabled-by-default product line after SPEC-010A acceptance; PR #83 merged as `d5308aa0723c48b670e88c6814b4e4a90892d74f` and HAL UAT passed with run id `SPEC-010B-HAL-20260606040404`. SPEC-012B added offline harness-gardening drift guards after real two-product-line behavior; PR #84 merged as `77c6ce44bb10ac89b43ffcaab2b9ea35e7ea2f39`. SPEC-011 is now active as the optional security adapter setup on branch `011-crabtrap-honeypot`. The SPEC-009C family is the first practical self-hosting gate, split into ingest, triage handoff, remediation-to-owner, and merge reconciliation so each PR is reviewable. SPEC-009D bridges pilot smoke to formal run-state by emitting the reviewable lifecycle packet. SPEC-009E turns pilot eligibility/evidence into operator-visible read-only surfaces. SPEC-009F owns production routing/evidence for non-remediation triage outcomes. SPEC-012A makes repo/process truth discoverable from checked-in docs. SPEC-010A extracts the reusable seeder from the Paddock-specific path. SPEC-013A-D own run-state, GitHub sync automation, claim/reconciliation, retry API authority, and claim-control operator UX. SPEC-014A-F execute already-claimed work and must not own tracker truth, successor selection, governance, or auto-merge policy. SPEC-015A owns the operator information architecture split between agent runtime management and workflow orchestration management.

SPEC-013A1 implementation, PR #60, retrospective, and operator-confirmed post-merge UAT are complete on `main`. SPEC-013B implementation, PR #62, HAL deployment promotion, and post-merge HITL UAT are complete on `main`. SPEC-013C PR #63 and SPEC-014A PR #64 are merged, deployed to HAL, and closed out with target UAT evidence on `main`. SPEC-013D PR #65 is merged to `main` with local manual browser UAT, Docker-backed visual evidence, archive/status hygiene, and HAL target UAT recorded. SPEC-014B PR #76 is merged, deployed to HAL, and closed out with target UAT evidence on `main`; SPEC-014C PR #79 is merged with Phase 7, HAL branch UAT, post-implementation closeout, and archive/status hygiene complete. SPEC-010B PR #83 is merged with local/HAL Product Line B smoke evidence, cleanup proof, and archive/status hygiene complete. SPEC-012B PR #84 is merged with process/tooling-only guardrail evidence and no live mutation.
...
- SPEC-010A is complete after PR #59 and 2026-05-22 post-merge UAT; SPEC-010B is complete after PR #83 and HAL Product Line B smoke evidence; SPEC-011 is now active on branch `011-crabtrap-honeypot` and should remain isolated to the optional security adapter scope.
- SPEC-012B is complete after PR #84 and now provides offline drift-guard recommendations for future cleanup lanes.
- SPEC-009F, SPEC-013A, SPEC-013A1, SPEC-013B, SPEC-013C, SPEC-013D, SPEC-014A, SPEC-014B, and SPEC-014C are complete after their recorded merge and UAT/verification gates.
...
- SPEC-014D may proceed after SPEC-014B/C completion when adapter modules, fixtures, and deployment docs stay isolated from retention/intervention/IA follow-ups.
...
| R10 Additive schema changes mistaken for low-risk changes | All schema phases | Install compatibility labeling; roadmap marks schema/state impact before implementation |
| R11 OpenClaw health electricity integration leaks OpenClaw-node assumptions into generic paths | Phase 7 | Operator-specific optional adapter, absent-safe runtime checks, and no v1 schema migration for health costs |
...
- **Self-hosting pilot** (Phases 8B-8D) — flip `PILOT_PADDOCK_E2E` OFF; workspace/templates remain, but pilot auto-chain stops; operator can fall back to explicit task assignment (Pattern 1).
...
## Install Compatibility Checklist (every PR)
...
- [ ] Does this PR rename any column in `workspaces`, `projects`, `tasks`, or `agents`? If yes, STOP unless a compatibility/rollback decision is recorded — DB renames are not automatically additive-safe.
- [ ] Does this PR modify shared shell/authentication files (`src/app/layout.tsx`, `src/lib/auth.ts`, etc.) in a way that would create broad regression risk? If yes, isolate the change to a new file or extend via hooks.
...
- [ ] If this PR is Symphony-inspired, does it preserve the Paddock stack and SpecKit task-chain authority rather than importing the Elixir prototype, Linear-only assumptions, or a general distributed scheduler?
