...
- **Explicit output-failure recovery**: missing/invalid-output recovery is an operator-only retry action, not a normal status update. Operators may correct `tasks.resolution`, then call `POST /api/tasks/[id]` with `{ "action": "retry_chain_advancement" }`. Ordinary `PUT /api/tasks/[id]` updates, including changing a failed task to `done`, MUST NOT implicitly rerun `advanceTaskChain`. The original failure activity stores `template_output_schema_sha256`, `template_routing_rules_sha256`, and `template_next_slug_sha256`. Retry uses the current `workflow_templates` row and recomputes those hashes; if any hash differs, the action returns `409 Conflict` until the operator retries with `{ "action": "retry_chain_advancement", "confirm_template_drift": true }`. The retry action records recovery provenance in `activities.data` with `recovery_action='retry_chain_advancement'`, `retry_attempt`, `previous_reason_code`, relevant task/template/chain ids, original/current template hashes, `template_drift_detected`, `template_drift_confirmed`, and a SHA-256 hash of corrected resolution when available; it does not duplicate the full corrected output into activity data. It validates current `tasks.resolution`, leaves the parent `failed` if validation still fails, and only restores the parent to terminal success once validation passes; after that point normal route, stall, termination, duplicate-successor, and successor-creation semantics apply using the current template.
