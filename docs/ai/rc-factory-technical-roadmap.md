...
- **Scheduler extension**: new `advanceTaskChain` function invoked only at live terminal-success transition points (Aegis approval in `runAegisReviews` and operator approval in `POST /api/quality-review`) with idempotent duplicate-successor prevention.
...
- **Template UI**: extend the live workflow-template editor in `src/components/panels/orchestration-bar.tsx`, `src/app/api/workflows/route.ts`, and `createWorkflowSchema` in `src/lib/validation.ts` with `slug`, `output_schema`, `routing_rules`, `next_template_slug`, `produces_pr`, `external_terminal_event`, and `allow_redacted_artifacts` fields while preserving existing operator-only write authorization.
...
- `src/app/api/workflows/route.ts`, `src/lib/validation.ts` — persistence and validation for workflow-template chain fields
- `src/types/workflow-template.ts` — add new field types
- `package.json`, `pnpm-lock.yaml` — add pinned direct `ajv` and `jsonpath-plus` dependencies and keep `safe-regex` as a pinned direct dependency; CI verifies all three are direct and lockfile-pinned, runs `safe-regex` over all referenced schemas, and records `pnpm audit` evidence
...
- [P3-AC8] `ajv`, `jsonpath-plus`, and `safe-regex` are present as direct pinned dependencies in `package.json` and `pnpm-lock.yaml`; ripgrep finds no transitive-only imports of any of them, and `pnpm audit` evidence is recorded before merge.
