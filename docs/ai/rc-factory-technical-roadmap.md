...
- **Active after SPEC-009D:** SPEC-009E is now set up on branch `009e-pilot-evidence-surfaces` by operator selection to continue the direct self-hosting evidence-surface path. SPEC-012A remains the highest-priority unblocked run-state prerequisite because it feeds SPEC-013, while SPEC-010A and SPEC-011 remain parallel options because they touch the generic seeder and optional security adapter respectively.
