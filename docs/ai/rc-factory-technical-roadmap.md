...
| 8B | Paddock product-line seed + flag activation | Yes | `PILOT_MISSION_CONTROL_E2E` | Phase 8C, Phase 9A |
| 8C | GitHub-linked Paddock pilot smoke | Pilot gate | `PILOT_MISSION_CONTROL_E2E` | Phase 8D, Phase 9B |
| 8D | Pilot review packet + lifecycle snapshot | Yes | `PILOT_MISSION_CONTROL_E2E` | Phase 11A |
...
| SPEC-009B | 8B | Paddock Product-Line Seed and Flag Activation | mission-control-seed | Complete | P0 | SPEC-009A, SPEC-006, SPEC-008 | SPEC-009C1, SPEC-010A | Phase 8B |
...
| SPEC-009B | Paddock is seeded as Product Line A without launching work | `PILOT_MISSION_CONTROL_E2E` seed scope only | Run seed twice on target deployment, inspect workspace/departments/agents/repo/templates/flags/governance plus separate Issue Triage and Issue Remediation workflow families, and confirm no issue claim/dispatch |
| SPEC-009C1 | One GitHub issue enters the pilot as an eligible Paddock task | `PILOT_MISSION_CONTROL_E2E` for one real or synthetic issue | Ingest or create the pilot issue, prove GitHub is tracker-of-record, verify eligibility, and confirm local-only tasks cannot enter the pilot |
...
| SPEC-009F | Production triage outcomes route to the right non-remediation lanes | `PILOT_MISSION_CONTROL_E2E` product-line scope for v1 unless Clarify proves a dedicated flag is required | Drive `NEEDS_SPEC`, needs-human, needs-specialist, duplicate, obsolete, and invalid fixtures and inspect correct lane/evidence without a remediation successor |
...
- **Autopilot notes:** OpenClaw is the current application harness choice, not a product requirement. If OpenClaw is absent or disabled, Paddock must still support Mission-Control-owned or other external-harness paths through the same registry. Imported OpenClaw agents start as visible unassigned inventory; role/domain workspace files should stay generic, and product/task context belongs in Paddock assignment/run packets. Before Specify or Plan, fetch the current external context and document which Harness Engineering/Symphony contracts are implemented, adapted, or explicitly deferred.
