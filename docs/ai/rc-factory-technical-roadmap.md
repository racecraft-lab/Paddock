...
| SPEC-013D | 11D | Claim-Control Operator UX | claim-control-operator-ux | UAT Pending | P1 | SPEC-013C | SPEC-014C | Phase 11D |
...
- **Control-plane path:** SPEC-013A, SPEC-013A1, SPEC-013B, and SPEC-013C are complete after their recorded merge and UAT evidence. SPEC-013D implementation is merged and archive-recorded, with deployed target UAT pending. SPEC-014A-D own execution inside already-claimed work. These completed specs do not launch harnesses.
- **Runner path:** SPEC-014A is complete after post-merge UAT. SPEC-014B is the next runner-path setup candidate because it establishes the adapter manifest, fake registry, and first operator-visible runtime inventory references. SPEC-014C and SPEC-014D then run in parallel if they do not touch the same adapter files.
...
- SPEC-009F, SPEC-013A, SPEC-013A1, SPEC-013B, SPEC-013C, and SPEC-014A are complete after their recorded merge and UAT/verification gates; SPEC-013D is implementation-merged with archive/status hygiene complete and deployed target UAT pending; SPEC-014B is now unblocked after SPEC-014A, and SPEC-014C waits for SPEC-014B plus SPEC-013D target UAT acceptance.
