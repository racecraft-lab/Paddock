...
- **Current active setup:** SPEC-013B is in progress on branch `013b-claim-reconciliation` after SPEC-013A1 acceptance. No active implementation workflow should be treated as in progress on `main` for SPEC-010A, SPEC-013A, or SPEC-013A1. SPEC-010B is also unblocked by SPEC-010A acceptance but still depends on Product Line B preflight cleanup. SPEC-011 remains a parallel option when file ownership stays disjoint because it touches the optional security adapter.
...
- **Control-plane path:** SPEC-013A and SPEC-013A1 are complete after post-merge UAT; SPEC-013B is the next claim/reconciliation slice before retry state proceeds. These specs do not launch harnesses.
...
SPEC-013A1 implementation, PR #60, retrospective, and operator-confirmed post-merge UAT are complete on `main`. SPEC-013B setup is active on branch `013b-claim-reconciliation`; SPEC-010B may also start after Product Line B preflight cleanup, while SPEC-012B continues to wait for SPEC-010B.
...
- SPEC-009F, SPEC-013A, and SPEC-013A1 are complete after their recorded merge and UAT gates; SPEC-013B is the next control-plane dependency before retry/debug and sandbox work proceed.
