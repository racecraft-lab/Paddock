...
| SPEC-013D | 11D | Claim-Control Operator UX | claim-control-operator-ux | In Progress | P1 | SPEC-013C | SPEC-014C | Phase 11D |
...
- **Control-plane path:** SPEC-013A, SPEC-013A1, SPEC-013B, and SPEC-013C are complete after post-merge UAT. SPEC-013D is active and owns the minimal operator UX needed to make claim controls usable before first real harness operation. SPEC-014A-D own execution inside already-claimed work. These completed specs do not launch harnesses.
...
SPEC-013A1 implementation, PR #60, retrospective, and operator-confirmed post-merge UAT are complete on `main`. SPEC-013B implementation, PR #62, HAL deployment promotion, and post-merge HITL UAT are complete on `main`. SPEC-013C PR #63 and SPEC-014A PR #64 are merged, deployed to HAL, and closed out with target UAT evidence on `main`. SPEC-013D is scaffolded on `013d-claim-control-operator-ux` and blocks SPEC-014C operational adoption until operator UX acceptance is complete; SPEC-014B waits for SPEC-014A and is now unblocked. SPEC-010B may also start after Product Line B preflight cleanup, while SPEC-012B continues to wait for SPEC-010B.
...
- SPEC-009F, SPEC-013A, SPEC-013A1, SPEC-013B, SPEC-013C, and SPEC-014A are complete after their recorded merge and UAT gates; SPEC-013D is active after SPEC-013C to close the operator UX gap before SPEC-014C, and SPEC-014B is now unblocked after SPEC-014A.
