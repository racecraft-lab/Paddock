...
| SPEC-010B | 9B | Product Line B Onboarding Smoke | product-line-b-smoke | In Progress | P2 | SPEC-009C4, SPEC-010A | SPEC-012B | Phase 9B |
...
- **Current active setup:** No SPEC-013C, SPEC-013D, SPEC-014A, SPEC-014B, or SPEC-014C implementation workflow remains active after PR #63/#65/#64/#76/#79 merge and recorded post-implementation evidence. SPEC-014D/E/F and SPEC-015A are pending follow-ons; SPEC-015A is the recorded IA/UI follow-up from SPEC-014C HAL browser UAT because workflow contracts/templates/pipelines need a first-class operator page instead of being stacked above Agent Squad on `/agents`. SPEC-010B setup is in progress on branch `010b-product-line-b-smoke` after HAL Product Line B preflight passed with no live DB mutation or conflicting Product Line B residue, and the branch now carries SPEC-010B artifacts through G6 after merging latest `main`. SPEC-011 remains a parallel option when file ownership stays disjoint because it touches the optional security adapter.
...
- **Scale/doc parallel path:** SPEC-010A is complete after post-merge UAT; SPEC-010B setup can proceed now that SPEC-009C4 and SPEC-010A are complete and HAL preflight is clean; SPEC-012B waits for two-product-line reality from SPEC-010B.
...
- **Next setup:** SPEC-013B, SPEC-013C, SPEC-013D, SPEC-014A, and SPEC-014B are complete after merge and recorded UAT evidence. SPEC-014C is active for the first real harness adapter lane. SPEC-010B setup is in progress after clean HAL Product Line B preflight evidence. SPEC-011 remains an optional parallel start only when file ownership stays disjoint.
...
- SPEC-010A is complete after PR #59 and 2026-05-22 post-merge UAT; SPEC-010B setup is in progress after clean Product Line B preflight; SPEC-011 may also start when file ownership stays disjoint.
