...
**Current roadmap note:** SPEC-001, SPEC-002, and SPEC-002A are complete on `main`. SPEC-003 implementation is complete on branch `003-global-aegis` and is awaiting PR review/merge; downstream specs remain pending until SPEC-003 lands.
...
- **Status:** In Progress
- **Branch status:** Implementation complete on branch `003-global-aegis`; awaiting PR review/merge.
- **Priority:** P1
...
- **Implementation evidence:** Complete on branch `003-global-aegis` on 2026-04-28. G7 passed with all 21 generated tasks checked. Evidence includes `src/lib/aegis.ts`, feature-flagged workspace-first/global-first resolver behavior, legacy fallback, idempotent `aegis_local_shadowed` activity writes, `runAegisReviews` using `getAegis(db, task.workspace_id)`, strict-scope coverage in `tsconfig.spec-strict.json` and `eslint.config.mjs`, focused Vitest passing 6 files / 68 tests, `pnpm typecheck` passing, `pnpm lint` passing with 0 errors and 10 pre-existing warnings, static guardrails passing with zero matches, and `pnpm build` passing with network access for Google Fonts. Full `pnpm test` remains blocked by baseline environment issues: GPG-agent signing failures in `gnap-sync.test.ts` and a provisioner socket timeout.
