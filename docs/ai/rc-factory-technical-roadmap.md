...
- [P3-AC4] With a bound workflow template that has `output_schema` set and missing `tasks.resolution` or INVALID agent output, the parent task transitions to `failed`, an activity records the validation failure with exact `data.reason_code` (`task_pipeline_output_missing` or `task_pipeline_output_invalid`), and no successor is created.
