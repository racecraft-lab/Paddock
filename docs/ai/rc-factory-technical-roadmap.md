...
- [P3-AC4] With a bound workflow template that has `output_schema` set and missing `tasks.resolution` or INVALID agent output, the parent task transitions to `failed`, an activity records the validation failure with exact `data.reason_code` (`task_pipeline_output_missing` or `task_pipeline_output_invalid`) plus template schema/routing hashes, and no successor is created. Recovery requires the operator-only retry action; ordinary `PUT /api/tasks/[id]` re-marking to `done` does not rerun chain advancement. The retry action validates corrected `tasks.resolution`, detects template hash drift, requires `confirm_template_drift=true` before retrying with changed current rules, fails closed with `409 Conflict` and no side effects if selected failure/stall hash provenance is missing, leaves the parent `failed` on repeated validation failure, and restores terminal success before normal chain routing only after validation passes.
