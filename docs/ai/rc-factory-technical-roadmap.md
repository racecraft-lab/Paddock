...
- **Explicit output-failure recovery**: missing/invalid-output recovery is an operator-only retry action, not a normal status update. Operators may correct `tasks.resolution`, then call `POST /api/tasks/[id]` with `{ "action": "retry_chain_advancement" }`. Ordinary `PUT /api/tasks/[id]` updates, including changing a failed task to `done`, MUST NOT implicitly rerun `advanceTaskChain`. The retry action records recovery provenance in `activities.data` with `recovery_action='retry_chain_advancement'`, `retry_attempt`, `previous_reason_code`, relevant task/template/chain ids, and a SHA-256 hash of corrected resolution when available; it does not duplicate the full corrected output into activity data. It validates current `tasks.resolution`, leaves the parent `failed` if validation still fails, and only restores the parent to terminal success once validation passes; after that point normal route, stall, termination, duplicate-successor, and successor-creation semantics apply.
...
- [P3-AC4] With a bound workflow template that has `output_schema` set and missing `tasks.resolution` or INVALID agent output, the parent task transitions to `failed`, an activity records the validation failure with exact `data.reason_code` (`task_pipeline_output_missing` or `task_pipeline_output_invalid`), and no successor is created. Recovery requires the operator-only retry action; ordinary `PUT /api/tasks/[id]` re-marking to `done` does not rerun chain advancement. The retry action validates corrected `tasks.resolution`, leaves the parent `failed` on repeated validation failure, and restores terminal success before normal chain routing only after validation passes.
