...
- **Current active setup:** No SPEC-013C, SPEC-013D, SPEC-014A, or SPEC-014B implementation workflow remains active after PR #63/#65/#64/#76 merge and recorded post-implementation evidence. SPEC-014C is the active runner-path setup/autopilot lane for the first real harness adapter pilot; it must reuse SPEC-014B's fake manifest/runtime-inventory boundary and avoid duplicating SPEC-013B claim/reconciliation authority, SPEC-013C retry semantics, or SPEC-013D task-detail operator UX. SPEC-010B setup is in progress on branch `010b-product-line-b-smoke` after HAL Product Line B preflight passed with no live DB mutation or conflicting Product Line B residue. SPEC-011 remains a parallel option when file ownership stays disjoint because it touches the optional security adapter.
