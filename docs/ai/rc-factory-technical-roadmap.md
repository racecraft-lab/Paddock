...
| SPEC-013C | 11C | Retry/Backoff and Debug API Surfaces | retry-debug-surfaces | In Progress | P1 | SPEC-013B | SPEC-013D | Phase 11C |
| SPEC-013D | 11D | Claim-Control Operator UX | claim-control-operator-ux | Pending | P1 | SPEC-013C | SPEC-014C | Phase 11D |
| SPEC-014A | 12A | Sandbox Ownership and Lifecycle Contract | sandbox-lifecycle-contract | In Progress | P1 | SPEC-013B | SPEC-014B | Phase 12A |
...
- **Current active setup:** No SPEC-013B implementation workflow remains active after PR #62 merge, HAL deployment promotion, and post-merge HITL UAT. SPEC-013C is implemented on branch `013c-retry-debug-surfaces` and remains In Progress until PR merge, target deployment promotion, and post-merge API-and-audit UAT. SPEC-013D is the required operator UX follow-up before first real harness operation, and SPEC-014A setup is active on branch `014a-sandbox-lifecycle-contract` for sandbox-lifecycle planning only; none of these should duplicate SPEC-013B claim/reconciliation authority. SPEC-010B is also unblocked by SPEC-010A acceptance but still depends on Product Line B preflight cleanup. SPEC-011 remains a parallel option when file ownership stays disjoint because it touches the optional security adapter.
...
- **Control-plane path:** SPEC-013A, SPEC-013A1, and SPEC-013B are complete after post-merge UAT. SPEC-013C now owns the backend retry/debug API authority implementation branch; SPEC-013D owns the minimal operator UX needed to make those controls usable before first real harness operation. SPEC-014A-D own execution inside already-claimed work. These completed specs do not launch harnesses.
...
- **Implementation evidence:** Branch `013c-retry-debug-surfaces` implements M79 operator release reasons plus scoped idempotency replay storage, `POST /api/tasks/[id]/claim-control`, the optional `claim_control` read-model extension, bounded audit evidence, OpenAPI/API index updates, UAT scaffolding, and PR review packet evidence. Local verification passed under Node 22.22.2: focused SPEC-013C cluster 6 files / 39 tests, `pnpm typecheck`, `pnpm lint`, `pnpm api:parity`, `pnpm check:strict-scope`, full `pnpm test` outside sandbox with 308 files / 3190 tests, and `pnpm build` outside sandbox. Target deployment promotion and post-merge API-and-audit UAT remain required before status can move to Complete.
