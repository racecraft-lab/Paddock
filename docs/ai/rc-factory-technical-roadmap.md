...
- **Current active setup:** No SPEC-013B implementation workflow remains active after PR #62 merge, HAL deployment promotion, and post-merge HITL UAT. SPEC-013C is implemented on branch `013c-retry-debug-surfaces` and remains In Progress until PR merge, target deployment promotion, and post-merge API-and-audit UAT. SPEC-013D is the required operator UX follow-up before first real harness operation, and SPEC-014A is unblocked for sandbox-lifecycle planning only; none of these should duplicate SPEC-013B claim/reconciliation authority. SPEC-010B is also unblocked by SPEC-010A acceptance but still depends on Product Line B preflight cleanup. SPEC-011 remains a parallel option when file ownership stays disjoint because it touches the optional security adapter.
