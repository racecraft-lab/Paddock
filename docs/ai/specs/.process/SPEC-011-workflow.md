...
| Implement | `$speckit-implement` | Complete | Foundation through US4 + polish complete; post-verification and PR stack evidence recorded |
...
| `us3` | `T017`-`T021` | Complete | Commits `737f96d1` and `02cfab95`; stale, unsafe, oversized, malformed, unsigned, invalid-signature, unsupported, and replay cases reject without rejection-path activity rows |
| `us4` + polish | `T022`-`T032` | Complete | Commit `8e3aba38`; fixture UAT, forbidden-surface inspection, guardrails, typecheck, lint, full unit suite, build, and diff whitespace checks passed after feature-flag/product-line cascade alignment and SPEC-012B stale-status detector false-positive remediation |
