...
- Chain retry recovery uses only the explicit operator `retry_chain_advancement` action; ordinary failed-to-`done` updates must not rerun advancement. Failed-parent retry is eligible only for missing/invalid output and may restore terminal success only after validation passes. Terminal-success stall retry is eligible for routing rejection, routing timeout, target resolution stalls, and missing assignee; it keeps the parent terminal-success throughout and never marks it `failed`. Ineligible state/reason pairs return `409 Conflict` with no side effects, no hard retry-attempt cap is enforced, and retry uses current template rules only after schema/routing drift is absent or explicitly confirmed.
