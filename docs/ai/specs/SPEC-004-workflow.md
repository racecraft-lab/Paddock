...
- [ ] `advanceTaskChain` implements valid routing, missing-output failure, invalid-output failure, explicit output-validation retry recovery, explicit terminal-success advancement-stall retry recovery, latest eligible failure/stall activity selection with no `activity_id` override or older-activity replay, current-template execution with template-drift confirmation, no-hard-cap retry attempts with monotonic per-parent audit evidence shared across retry classes, ineligible retry `409 Conflict` no-side-effect behavior, fallback, missing-assignee stall, timeout/unresolved-target advancement stall, exact `activities.data.reason_code` and `recovery_class` metadata for every failure/stall/retry activity, termination, duplicate-successor retry no-op, transaction rollback, successor creation, first-hop parent lineage initialization, and successor lineage behavior at every live non-`done` to `done` transition for pipeline-bound tasks, including `runAegisReviews`, `POST /api/quality-review`, bulk `PUT /api/tasks`, and detail `PUT /api/tasks/[id]`; M62 enforces one successor per non-null `parent_task_id` at the DB layer with preflight and rollback evidence.
