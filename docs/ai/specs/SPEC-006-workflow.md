...
- Add outbound emission in `pushTaskToGitHub`: emit `area:<projects.area_slug>` alongside `pd:*` and `priority:*` only when `area_slug` is non-NULL. Triage project (if `area_slug='triage'`) emits `area:triage`. Projects without `area_slug` set emit no `area:*` label.
...
- [ ] P5-AC6: With the flag ON, `pushTaskToGitHub` emits `area:<projects.area_slug>` alongside `pd:*` and `priority:*` only when `area_slug` is non-NULL. Triage project (if `area_slug='triage'`) emits `area:triage`. Projects with NULL `area_slug` do not emit any `area:*` label.
...
- Extend `src/lib/github-sync-engine.ts`:
  ...
  - In `pushTaskToGitHub`, emit `area:<projects.area_slug>` alongside `pd:*`/`priority:*` only when the resolved project's `area_slug` is non-NULL.
...
- Do not change existing `pd:*` or `priority:*` label generation.
...
- Open Question 6 (no `area_slug` outbound behavior): Confirm the design concept Q6 decision: projects with NULL `area_slug` and flag ON emit no `area:*` label outbound. Do NOT fall back to `pd:no-area`. Confirm this remains the position.
```
...
- `initializeLabels(repo)` (no `workspaceId`) call from existing code paths still creates only `pd:*` and `priority:*` labels.
