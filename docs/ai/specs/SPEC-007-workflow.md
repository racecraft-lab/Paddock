...
- Successor dispatch (flag ON): `task.input.input_artifacts: Array<{ id, type, sha256, preview_text, storage_kind, byte_size }>`. Raw content only via authenticated MC artifact-read API. (Q14)
...
- [P6-AC6] Agent output can publish inline JSON, inline Markdown, and file-backed artifacts from a private sandbox into MC artifact storage.
...
- Confirm the exact rule list and source pin for MC Secret Detector v1 (per Design Concept; gitleaks v8.18.0 baseline + MC additions). Identify which rules ship in `src/lib/secret-detector.rules.ts` vs. deferred to v2.
...
| S3-Q5 (Cursor format) | 1 | [codebase] | Resolved without consensus (grep confirmed no existing cursor pagination in `src/app/api/`). Opaque base64url JSON cursor `{triaged_at, id}` + response shape `{rows, next_cursor, has_more}`. New MC-wide convention. | parent (verified by grep) |
