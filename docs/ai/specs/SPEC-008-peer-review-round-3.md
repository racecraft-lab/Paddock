...
- Claude Code's native OTel emits `claude_code.user_prompt` events when `OTEL_LOG_USER_PROMPTS=1`; the operator-facing setup path is now summarized in `docs/observability/setup.md` and `docs/observability/provider-tos-considerations.md`, so those docs must warn that this flag exfiltrates conversation content into Paddock's SQLite.
