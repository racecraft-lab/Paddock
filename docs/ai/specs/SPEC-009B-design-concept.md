...
- Seed Paddock itself as Product Line A with a non-facility `paddock` workspace while preserving `facility` as the Facility/global aggregate support row.
- Keep SPEC-009B Paddock-specific and reviewable; defer reusable product-line seeding to SPEC-010A.
...
- Enable Phase 1-7 pilot prerequisite flags and `PILOT_PADDOCK_E2E` only for the Paddock product-line scope.
...
- Automatically deleting FocusEngine or any other non-Paddock sync state from the seed script (Q5).
...
### Q1. For SPEC-009B, should the seed be Paddock-specific or generic?

**Branch:** Scope boundary

**Recommended answer:** Paddock-specific fixture/script.
...
**Recommended answer:** Upsert a non-facility workspace with slug `paddock` and display name `Paddock`, attached to the existing/default tenant, while preserving the seeded Facility workspace as aggregate/global scope.
...
### Q5. How should SPEC-009B handle non-Paddock GitHub sync/project state if it exists when the seed runs?
...
**Alternatives offered:**
- Automatically unlink/delete non-Paddock sync state: convenient, but destructive and risky.
- Ignore non-Paddock sync state: fastest, but conflicts with the desired "only Paddock issues synced" target.
...
**Recommended answer:** Enable only the Phase 1-7 flags needed for the pilot on the `paddock` workspace plus `PILOT_PADDOCK_E2E`; do not enable future runner/sandbox flags.
> SPEC-009B/009C need the merged primitives but must not jump ahead into SPEC-013 or SPEC-014 runner scope.

**Alternatives offered:**
- Only `PILOT_PADDOCK_E2E`: narrow, but may not activate the merged primitives the pilot needs.
...
**Recommended answer:** Use the repo-owned Paddock workflow contract as the source, apply/import it into `workflow_templates` for the `paddock` workspace, and verify the seeded slugs for both families.
...
Run `$speckit-autopilot docs/ai/specs/SPEC-009B-workflow.md` from the `009b-paddock-seed` worktree after setup commits and pushes this design concept and workflow file.
