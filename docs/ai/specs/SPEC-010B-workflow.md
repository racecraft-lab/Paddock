...
| Focused seed and smoke tests | Complete | `pnpm test src/lib/__tests__/product-line-b-seed.test.ts src/lib/__tests__/product-line-b-smoke.test.ts src/lib/__tests__/product-line-seed.test.ts src/lib/__tests__/product-line-seed-cli.test.ts`; UAT fix rerun `pnpm test src/lib/__tests__/product-line-b-smoke.test.ts` | Pre-HAL combined run passed: 4 files, 69 tests. HAL UAT found and fixed synthetic evidence run-id projection; focused smoke rerun passed: 17 tests. Covers Product Line B config/preflight/apply/verify/idempotency, reusable Paddock seed regression, synthetic smoke, DB-backed enable/disable/cleanup proof, redaction, Product Line A isolation, scoped evidence, and SPEC-014C boundary |
...
| HAL UAT | Complete | `/home/linuxbrew/.linuxbrew/opt/node@22/bin/node --experimental-strip-types scripts/seed-product-line.ts ...` and `scripts/spec-010b/product-line-b-smoke.ts ...` from isolated HAL UAT directory `/home/fredrick-gabelmann/paddock-uat/spec-010b-20260606040404` | Run id `SPEC-010B-HAL-20260606040404`; backup `/home/fredrick-gabelmann/paddock-evidence/spec-010b/paddock-before-SPEC-010B-HAL-20260606040404.db`; preflight `READY` with no-mutation hash `ee37dae9251315c4ecbd5a552dc7309b3e784953d345c0c8d475ac7d74de3ae4`; apply `SEEDED` workspace `31`; verify `VERIFIED` with no drift; repeated apply without allowance refused; allowed reapply stayed `already_valid`; smoke enable/disable closed cleanly; final cleanup proof zeroed sync-owner, GitHub-sync, dispatch-eligible, remaining-smoke, and unintended side-effect counters; final DB state `product-line-b` disabled with 6 projects, 0 sync-enabled, 0 sync-owner. Browser loaded `https://hal.boreal-fir.ts.net:3000/` dashboard shell; unauthenticated `/api/status` returned `401`. `/usr/bin/node` was not service-compatible on HAL because its ABI was 137 while active `better-sqlite3` was ABI 127 |
