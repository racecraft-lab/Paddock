...
1. **Facility operator** (`visual-reviewer` today) — runs multiple product lines, needs focus mode + cross-product awareness. Primary user.
...
- **FR-I4:** Point FocusEngine workspace's GitHub repo at `visual-reviewer/focusengine` (or `racecraft-lab/focusengine`, whichever is canonical at rollout).
