...
4. **Dev-first, flag-scoped canary on live.** `<dev-worktree>` (`<feature-branch>`) is the dev worktree where changes are written and committed. `<live-worktree>` (`main`) IS the live service — it serves the compiled `.next/standalone` bundle that `next-server` (PID in `/proc/<n>/cwd`) runs from. The "canary" is NOT a separate environment; it's a feature flag flipped for ONE workspace (e.g., facility or a dedicated test workspace) on the live service, validated, then promoted to wider workspaces.
...
- `next-server` runs from `<live-worktree>/.next/standalone` → **sync worktree is live**.
- `openclaw-gateway` runs from `~/openclaw-release-publish-v2026.4.21.7` → **independent of MC worktrees**.
- DB: `<dev-worktree>/.data/mission-control.db` (shared across worktrees; auto-backed-up nightly at 03:00 UTC to `.data/backups/`).
- **Mission Control is user systemd-managed** via `mission-control.service` with `worktree.conf` pointing at `<live-worktree>`; OpenClaw gateway is independent. Restart MC with `systemctl --user restart mission-control.service` after deploy/build.
...
- [ ] Verify most-recent auto-backup exists: `ls -la <dev-worktree>/.data/backups/ | tail -3`.
- [ ] Take an ad-hoc pre-migration snapshot: `sqlite3 <dev-worktree>/.data/mission-control.db ".backup <dev-worktree>/.data/backups/mc-pre-m53.db"`.
- [ ] Capture schema baseline: `sqlite3 <dev-worktree>/.data/mission-control.db ".schema" > /tmp/schema-pre.txt`.
- [ ] Confirm no running long-writes: `lsof <dev-worktree>/.data/mission-control.db` — expect only next-server.
- [ ] Confirm `mission-control.service` drop-ins still point at `<live-worktree>`, then relaunch cleanly with `systemctl --user restart mission-control.service` after migrations.

### Post-migration verification (Phase 0)

- [ ] `sqlite3 <dev-worktree>/.data/mission-control.db "PRAGMA integrity_check;"` returns `ok`.
- [ ] `sqlite3 <dev-worktree>/.data/mission-control.db "SELECT name, scope FROM agents WHERE scope='global';"` returns 3 rows.
- [ ] `sqlite3 <dev-worktree>/.data/mission-control.db "SELECT slug FROM workspaces WHERE slug='facility';"` returns 1 row.
...
1. Snapshot the live database: `cp <live-worktree>/data/mission-control.db{,.pre-m53.bak}`.
