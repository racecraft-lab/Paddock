...
- **FR-D2a:** Recovery from an output-validation failure is explicit, not a normal status re-mark. Editing `tasks.resolution` or changing a failed task back to `done` through the ordinary `PUT /api/tasks/[id]` update MUST NOT implicitly rerun chain advancement. After an operator corrects the output, they trigger an operator-only `POST /api/tasks/[id]` action with `{ "action": "retry_chain_advancement" }`. The action reuses `advanceTaskChain` in recovery mode, records recovery provenance in `activities.data`, and revalidates the current `tasks.resolution` before any route evaluation or successor creation. Retry uses the current `workflow_templates` row, but drift is explicit: the original failure activity stores `template_output_schema_sha256`, `template_routing_rules_sha256`, and `template_next_slug_sha256`; retry recomputes those hashes from the current template and returns `409 Conflict` unless the operator retries with `{ "action": "retry_chain_advancement", "confirm_template_drift": true }` when any hash differs. Recovery activity data includes `recovery_action='retry_chain_advancement'`, `retry_attempt`, `previous_reason_code`, relevant task/template/chain ids, original/current template hashes, `template_drift_detected`, `template_drift_confirmed`, and a SHA-256 hash of the corrected resolution when available; it MUST NOT duplicate the full corrected output into `activities.data`. If output validation still fails, the parent remains `failed` and no successor is created. If output validation passes, the action restores the parent to terminal success and then applies normal route, stall, termination, idempotency, and successor-creation semantics using the current template.
...
- **FR-D4b:** Every SPEC-004 chain failure or advancement-stall activity MUST include a human-readable `activities.description` and stable machine-readable metadata in `activities.data` JSON. `activities.data.reason_code` is required and must use one of the SPEC-004 codes: `task_pipeline_output_missing`, `task_pipeline_output_invalid`, `task_pipeline_routing_expression_rejected`, `task_pipeline_routing_budget_exceeded`, `task_pipeline_target_missing`, `task_pipeline_target_disabled`, `task_pipeline_target_duplicate`, `task_pipeline_target_cross_workspace`, or `task_pipeline_successor_assignee_missing`. The data payload also includes relevant non-secret context such as `parent_task_id`, `workflow_template_id`, `workflow_template_slug`, `target_template_slug`, `chain_id`, and `chain_stage` when available. Tests assert exact reason codes so operator filters, dashboards, and later automation can depend on them.
