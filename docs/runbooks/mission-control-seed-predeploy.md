...
- Full seed twice plus verify-mode evidence on a clean eligible target: covered by `src/lib/__tests__/mission-control-seed/evidence.test.ts`; two apply runs against a clean in-memory target produce the same identity hash and `verifyMissionControlSeed()` returns `ok: true`, `status: "verified"`.
