...
- Real UI journey gate: `tests/product-line-switcher-ui.spec.ts` must drive the running app with seeded Product Line data, attach screenshots for the core Facility/Product Line states and narrow mobile layouts, pass through `pnpm test:e2e:docker`, and pass `pnpm test:e2e:argos-metadata` so Argos uploads include Playwright test identity and spec-scoped tags before the PR branch is updated. Storybook visual coverage must pass `pnpm test:visual:argos-metadata` so Argos uploads include SPEC-002 story identity and tags.
