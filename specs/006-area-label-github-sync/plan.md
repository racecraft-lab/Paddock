...
**Regression-safety test gates** (from regression-safety checklist, derived from FR-002 + FR-052..FR-056):

- `pnpm test`, `pnpm test:e2e`, `pnpm typecheck`, and `pnpm lint` MUST pass after the SPEC-006 schema and code additions land. Existing test fixtures default `workspaces.feature_flags` to an empty/null JSON (flag OFF) so any pre-SPEC-006 test that does not explicitly opt in inherits flag-OFF behavior. Tests that need flag-ON MUST opt in explicitly by writing the flag into the workspace's `feature_flags` JSON in their setup; the new code MUST NOT introduce any global toggle that could flip pre-existing tests into flag-ON behavior implicitly.
- A mixed-tenant Vitest fixture (FR-052) seeds at least two workspaces — one flag-ON, one flag-OFF — and asserts the poller's selection query for each, end-to-end, in the same process. This fixture is the canonical guard against the mass-mode pitfall.
- An `EXPLAIN QUERY PLAN` snapshot harness captures the planner output for the canonical pre-existing legacy query set (e.g., agent-queue selection, task-list filter-by-workspace, search-by-id, sync-owner-less poller selection — exact SQL list pinned in tasks.md). The harness runs once before migration M62/M63 and once after, asserting that none of the four new indexes appears in the plan for any legacy query. Any planner delta between pre- and post-migration snapshots fails the test, surfacing the regression at CI rather than in production. Index creation is gated entirely behind the migration body (no startup-time `CREATE INDEX` calls), so the snapshot harness only needs to compare two states (pre-migration vs. post-migration) to certify the partial-index predicates (`WHERE is_repo_sync_owner=1`, `WHERE is_triage_project=1`, `WHERE github_issue_number IS NOT NULL AND area_routing_backfilled_at IS NULL`) cannot be selected by legacy queries that do not reference those predicates.
