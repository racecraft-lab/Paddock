...
- **FR-027a**: The `label_provisioning_failed` activity's `data` field MUST include: `workspace_id: number`, `github_repo: string`, `failed_labels: string[]`, `error_count: number`, `sample_error: string` (truncated, no PII or auth headers), and `trigger: 'connect' | 'area_slug_change' | 'bootstrap'`.
