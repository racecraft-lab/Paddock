...
- **FR-027**: GitHub API failures during `initializeLabels` (rate-limit, 4xx, network errors) MUST be caught per-label, logged, and not abort either the function itself or the larger sync run that called it. The function MUST return successfully even on partial failure. When one or more per-label failures occur in a single call, the system MUST aggregate them and write at most one `kind='label_provisioning_failed'` activity per `(workspace_id, github_repo)` per 24-hour window (throttle by querying the activity log for an existing row with `created_at > unixepoch() - 86400` before insert). All per-label failures from a single `initializeLabels` invocation collapse into one activity row, never one per failed label.
...
- **Activity**: A Mission Control activity log row. SPEC-006 introduces three new `kind` values (`area_routing_resolved`, `area_routing_unresolved`, `label_provisioning_failed`) and fixed `data` shapes per kind: routing decisions carry `area_labels`/`reason`/`source` plus issue context; label-provisioning failures carry an aggregated `failed_labels`/`error_count`/`sample_error`/`trigger` payload throttled to one per `(workspace_id, github_repo)` per 24h.
