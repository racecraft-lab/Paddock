...
| XIII. Defensive Boundaries, Trusting Interior | PASS | API boundaries (3 routes) catch and classify per the API Error Code Matrix (spec.md §"API Error Code Matrix"). Detector findings log rule id, never matched substring. Disposition INSERT failure is caught + activity row, never propagates. File-write `fs.link()` EEXIST/other-error branches handled per FR-022/FR-023. Trusted interior (`task-artifacts.ts` internal helpers) trusts type guarantees. |
