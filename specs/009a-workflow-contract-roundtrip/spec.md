...
- Q: What migration slot is reserved for diagnostics? A: Current repository state already uses M70, so diagnostics persistence must use additive migration `071_workflow_contract_diagnostics` with `docs/migrations/rollback-M71.sql`, unless a later concurrent merge takes M71 first and the migration is rebased per the repository migration reservation policy.
...
- **FR-049**: Diagnostics schema, if added, MUST use additive migration `071_workflow_contract_diagnostics` and ship a matching `docs/migrations/rollback-M71.sql`, unless migration-id collision requires a documented rebase.
