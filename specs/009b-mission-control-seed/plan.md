...
**Constraints**: Additive only; no new Product Line table; no `project_agent_assignments.workspace_id`; no destructive cleanup; operator cleanup docs require backup/export-first steps, explicit confirmation, and post-cleanup verification; no synthetic issue creation or ingestion; no claim/dispatch/scheduler/runner/sandbox state; no generic Product Line B seeder; raw credentials never appear in logs/evidence
...
3. Implement preflight scans that detect non-Mission-Control projects, linked tasks, GitHub repo config, cron issue-sync state, OpenClaw/gateway GitHub automation or product-line binding evidence, and operator-supplied `ssh hal` evidence. OpenClaw runtime agent inventory alone is not blocking residue. If residue exists, return a blocked result before mutation with redacted summaries and backup/export-first cleanup checklist references that require explicit operator confirmation for destructive cleanup and post-cleanup verification.
...
- Pre-deploy cleanup documentation requires backup/export proof, explicit operator confirmation for destructive cleanup, redacted cleanup evidence, and post-cleanup verification that only `racecraft-lab/mission-control` issue sync remains.
