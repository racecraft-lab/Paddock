...
2. **Given** synced projects, linked tasks, repo config, cron issue sync, OpenClaw/gateway GitHub automation, product-line binding evidence, or FocusEngine live project state for a non-Mission-Control product, **When** the preflight runs, **Then** the seed blocks with actionable cleanup instructions and performs no automatic deletion.
3. **Given** the known `ssh hal` FocusEngine cleanup concern, **When** deployment evidence is reviewed, **Then** the operator has backup/export-first cleanup checklist coverage for FocusEngine project state, tickets, GitHub sync, OpenClaw/gateway GitHub automation or product-line binding cleanup, and issue-sync cron before deployment while retained OpenClaw runtime agent inventory is not treated as residue by itself.
...
- Non-Mission-Control sync residue may include projects, tasks, repo config, cron entries, gateway GitHub automation, product-line bindings, or FocusEngine live state; detection must be actionable and non-destructive. OpenClaw runtime agent registrations alone are inventory, not cleanup residue.
...
- **FR-010**: The seed MUST detect non-Mission-Control synced projects, linked tasks, GitHub repo config, issue-sync cron state, OpenClaw/gateway GitHub sync or triage automation, product-line binding evidence, and FocusEngine live project residue before applying changes that depend on a clean Mission Control-only sync state.
...
- **FR-012**: Operator cleanup guidance MUST cover the known `ssh hal` FocusEngine project, tickets, GitHub sync configuration, OpenClaw/gateway GitHub automation or product-line binding cleanup targets, and issue-sync cron cleanup targets. It MUST state that OpenClaw runtime agent inventory alone may remain unless a separate operator decommission is approved.
...
- **FR-036**: Retained OpenClaw runtime agents in the pre-deploy runbook MUST be treated as visible unassigned runtime inventory, not Product Line A or Product Line B assignment. Operator guidance MUST preserve stable OpenClaw ids unless a safe rename path is verified, generalize display labels and workspace files away from FocusEngine/macOS/product-specific text, keep retained agents offline or otherwise not work-eligible, and verify zero project assignments, zero active/assigned tasks, and no GitHub sync/triage cron ownership before SPEC-009B apply. First-class runtime inventory UI/state belongs to SPEC-014B/SPEC-014D, not SPEC-009B.
...
- **Cleanup Residue**: Non-Mission-Control sync, project, cron, gateway GitHub automation, product-line binding, or FocusEngine state that blocks seed readiness until an operator cleans it up outside the seed. OpenClaw runtime agent registrations alone are not cleanup residue.
...
- **SC-005**: A target with non-Mission-Control sync, project, cron, gateway GitHub automation, product-line binding, or FocusEngine residue blocks before unsafe mutation, reports cleanup instructions that an operator can act on, returns `mutation_status: "not_mutated"`, and leaves before/after row and file snapshots unchanged for non-Mission-Control residue.
...
- **SC-010**: The pre-deploy checklist identifies the `ssh hal` FocusEngine cleanup targets, requires backup/export first, records explicit operator confirmation for destructive cleanup actions, and records verification that only `racecraft-lab/mission-control` issue sync remains before deployment. Retained OpenClaw runtime agent inventory does not violate this criterion when generalized, unassigned, not work-eligible, and free of FocusEngine GitHub automation.
...
- FocusEngine cleanup readiness is inferred from Mission Control sync/project/task state plus operator-visible cron, GitHub automation, and OpenClaw/gateway product-line binding evidence; FocusEngine is not a Mission Control domain entity in SPEC-009B. OpenClaw runtime agent registrations may remain as reusable runtime identities unless separately decommissioned.
