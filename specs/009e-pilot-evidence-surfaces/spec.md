...
- A user reloads, retries, or reopens the Evidence section after seeing `missing`, `stale`, `unavailable`, `redacted`, `quarantined`, `superseded`, or `deferred`; the UI may issue another read-only `GET` request, but must not trigger refresh, repair, backfill, artifact mutation, quarantine/supersession updates, or audit/event writes.
- The client previously rendered a different evidence state for the same section; the current API response remains authoritative, and the client must not retain or apply local override state that replaces, upgrades, downgrades, hides, or synthesizes evidence states.
- Stored evidence text contains Markdown links, autolink-looking URLs, raw HTML, or unsafe URL schemes such as `javascript:`, `data:`, `vbscript:`, or `file:`; the route/UI must render the stored value as inert text or safe metadata and must not turn it into an active link or executable markup.
- Packet names, GitHub references, source-map pointers, warning reason codes, or deferred labels are longer than the available Details width; the Evidence UI must avoid clipping or overlap and keep the full text keyboard-reachable and screen-reader discoverable.
- The operator reviews the Evidence section at narrow width or high zoom; eligibility, current stage, packet references, missing proof, warnings, deferred labels, and their links must remain readable or otherwise available in full without losing their category association.
...
- **FR-019**: The task evidence route MUST return `401` for unauthenticated reads, `400` for malformed workspace scope, existing `403` for forbidden explicit workspace scope, and masked `404 task_not_found` for missing tasks or tasks outside the authorized workspace scope. The route MUST map those boundary cases to exact `ErrorResponse.error` values: `unauthenticated` for `401`, `invalid_workspace_scope` for `400`, `forbidden_workspace_scope` for `403`, and `task_not_found` for masked `404`. The masked `404` response MUST use `task_not_found` for both nonexistent tasks and tasks outside authorized workspace scope and MUST NOT reveal which condition occurred.
- **FR-020**: The task evidence route MUST represent disabled or unavailable artifact storage as a section-level unavailable warning when other task-local evidence can still be returned; direct artifact routes are not changed by this requirement. In this aggregate task evidence response, the warning MUST use `code: "section_unavailable"`, `section: "packet_artifacts"`, and `reason: "artifact_storage_disabled"` when storage is disabled by configuration or `reason: "artifact_storage_unavailable"` when the storage dependency is temporarily unavailable. The response MUST keep non-artifact evidence readable when available, set the packet/artifact section state to `unavailable`, omit artifact content that cannot be resolved, and MUST NOT duplicate or alter direct artifact read-route semantics such as `503 artifact_store_disabled`.
...
- **FR-023**: The Evidence UI MUST expose status text, section headings, artifact/GitHub reference names, and async loading/error updates in an accessible way that does not depend on color alone. Eligibility, current stage, packet references, missing proof, warnings, and deferred sections MUST each have visible labels and screen-reader-accessible names or descriptions that identify the evidence category and current state.
...
- **FR-033**: The Evidence UI MUST preserve full text access for long packet names, GitHub references, source-map pointers, warning reason codes, and deferred labels. Visual truncation is allowed only when the full value remains available to keyboard and screen-reader users through accessible text, title/description, or an equivalent full-text disclosure.
- **FR-034**: The Evidence UI MUST reflow at narrow widths and high zoom without overlapping labels, values, links, controls, or status text. Long evidence references and machine-readable reason codes MUST wrap, truncate with full-text access, or use an overflow-safe container while remaining visually associated with the relevant evidence category.
- **FR-035**: The v1 evidence state model in `data-model.md` is authoritative. Every v1 response section MUST use only the state values allowed for that section by `data-model.md` §V1 Evidence State Model, and MUST NOT introduce UI-only state aliases for `available`, `missing`, `stale`, `incomplete`, `not_eligible`, `redacted`, `quarantined`, `superseded`, `unavailable`, or `deferred`.
- **FR-036**: SPEC-009E MUST be read-only end to end. The server route and helper MUST perform read-only retrieval only and MUST NOT persist evidence state, mutate task, artifact, packet, smoke, GitHub sync, quarantine, supersession, redaction, activity, audit, or pilot-status records, enqueue refresh/repair/backfill jobs, or trigger hidden evidence refresh. The client MAY keep ephemeral in-memory loading, error, and response-render state for the current task detail view, but MUST NOT persist evidence overrides or derived evidence state in `localStorage`, `sessionStorage`, IndexedDB, Cache Storage, cookies, URL parameters, Zustand persistence, or any application cache.
- **FR-037**: The Evidence UI MUST treat the API-provided evidence `state` for each section as authoritative. The UI MAY map states to labels, colors, copy, and accessible descriptions, but MUST NOT reinterpret, synthesize, merge, upgrade, downgrade, hide, override, or coerce states client-side; MUST NOT infer `missing`, `stale`, `incomplete`, `available`, or any other state from timestamps, absent fields, cached content, or neighboring sections; and MUST render unknown API state values as an explicit unsupported contract/error condition instead of coercing them to a known evidence state.
- **FR-038**: The task evidence route and Evidence UI MUST treat all stored evidence-derived strings, including Markdown packet names, preview text, warning details, source-map notes, and reason text, as untrusted display text. Stored values MUST NOT be parsed as raw HTML, script-capable markup, Markdown links, autolinks, or navigation triggers. Active links are allowed only when constructed from allowlisted typed references supplied by the task evidence contract: GitHub issue/PR references, artifact ids or hashes, source-map pointers, checklist anchors, or static UAT links. Typed URL references MUST be validated to an allowed protocol and expected destination family before rendering as links; unsafe schemes such as `javascript:`, `data:`, `vbscript:`, and `file:` MUST render as inert text.
...
- **SC-006**: 100% of tested Evidence UI states expose screen-reader-distinguishable labels or descriptions for eligibility, current stage, packet references, missing proof, warnings, and deferred sections.
- **SC-007**: At narrow viewport coverage used for UAT, and at browser zoom up to 400% when practical, long packet names, GitHub references, source-map pointers, warning reason codes, and deferred labels do not overlap or become inaccessible, and full text remains available to keyboard and screen-reader users.
