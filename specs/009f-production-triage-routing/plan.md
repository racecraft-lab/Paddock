...
- `src/lib/__tests__/triage-routing-payloads.test.ts` (created): RED coverage for four lane artifact families across six supported dispositions, bad schema/version/fields, sanitized validation-failure reasons, over-limit text, control characters, newline limits, safe text/reference stripping, query/fragment stripping, unsafe schemes, non-allowlisted destinations, proposed-label normalization, and unsupported outcomes.
