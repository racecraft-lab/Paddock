...
- **FR-017**: System MUST prove invalid-config no-mutation behavior by comparing `snapshot_before` and `snapshot_after` state using per-surface row counts plus stable ordered-JSON SHA-256 hashes formatted as `product-line-seed-snapshot-v1:sha256:<hex>` across product-line, department, assignment, workflow, governance, task, issue, activity, history, evidence, and GitHub sync surfaces.
...
- **RR-003**: The review packet MUST include before/after per-surface row counts plus stable snapshot hashes for invalid-config no-mutation and apply-twice idempotency.
...
- **SC-007**: No-mutation evidence proves failed invalid-config runs leave product-line, department, assignment, workflow, governance, issue, task, activity, history, evidence, and GitHub sync counts and snapshot hashes unchanged.
...
- **SC-009**: Explicit existing-target apply preserves 100% of non-owned issue, task, activity, history, comments, notifications, evidence, GitHub sync, manual workflow-template, governance audit, and unrelated feature-flag state in covered test scenarios.
