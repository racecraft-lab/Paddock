...
- Q: What normalized payload schema is accepted? -> A: The signed fixture uses a flat, strict `crabtrap_denial_summary.v1` schema. Required fields are `schema_version`, `source`, `event_id`, `signed_at`, `occurred_at`, `decision`, `method`, `url_host`, `url_path`, `reason_code`, `safe_request_hash`, `denial_count`, `actor_kind`, and `signature`. Optional fields are `source_instance_hash`, `actor_ref_hash`, context-approved `workspace_id`, context-approved `project_id`, `probe_kind`, `url_path_hash`, `distinct_host_count`, `distinct_path_count`, and `distinct_actor_count`. Unknown fields are rejected.
- Q: How are URL fields normalized? -> A: Accepted evidence persists only lowercased `url_host` and parsed pathname `url_path`. The normalized payload must not contain raw URL, full URL, scheme, userinfo, query, fragment, CR/LF, blank host/path, or secret-like path values. `/` is allowed. `CONNECT` and `TRACE` methods are unsupported in SPEC-011 and deferred to future live-proxy intake if needed.
- Q: How are actor and source identity represented? -> A: Activity producer identity is always Paddock-owned `crabtrap-adapter`, never payload-controlled. Payload identity is limited to bounded `source`, optional `source_instance_hash`, required `actor_kind`, and optional keyed `actor_ref_hash`; raw actor IDs, user IDs, and emails are forbidden. `workspace_id` and `project_id` are accepted only from approved adapter context, not blindly trusted from fixture payload.
- Q: What decision, method, and reason taxonomy applies? -> A: `decision` accepts only `deny`. `method` accepts only `GET`, `HEAD`, `POST`, `PUT`, `PATCH`, `DELETE`, and `OPTIONS`. `reason_code` accepts only `static_rule_denied`, `llm_policy_denied`, `fallback_denied`, `ssrf_blocked`, `rate_limited`, `policy_denied`, and `unknown_denied`; `policy_denied` and `unknown_denied` are last-resort normalized values and raw vendor reason text is never persisted.
- Q: What hash and count bounds apply? -> A: Accepted hash fields must match `sha256:<64hex>` and be derived only from safe canonical inputs; `actor_ref_hash` must be keyed if it represents a low-entropy actor identifier. `replay_key_hash` is adapter-derived persisted evidence only, not accepted from intake. Count fields are nonnegative safe integers from 0 through 1,000,000 inclusive.
- Q: Which activity field stores the CrabTrap event class? -> A: Paddock stores the event class in `activities.type`, so accepted evidence MUST use `activities.type='security_intrusion_detected'`; `kind` is not a schema column.
- Q: Where does accepted evidence land when no task is implicated? -> A: Accepted evidence lands as an existing-schema workspace activity: `entity_type='workspace'`, `entity_id=workspace_id`, and `workspace_id=workspace_id` when approved workspace context exists. If no approved workspace/project context exists, the adapter uses the real facility workspace row. Validated `project_id` may appear only in bounded `data`, not as an activity entity.
...
- Unknown normalized payload fields must be rejected because the accepted schema is flat and allowlisted.
...
- Replay detection must be scoped to the selected workspace/facility landing row and `data.replay_key_hash`.
...
- Raw/full URL fields, URL userinfo, URL query, URL fragment, raw headers, bodies, cookies, Authorization values, API keys, query secrets, provider payloads, and full audit rows must be rejected even when the signature is otherwise valid.
...
- **FR-006**: System MUST normalize accepted denial summaries through a flat, strict `crabtrap_denial_summary.v1` schema with required `schema_version`, `source`, `event_id`, `signed_at`, `occurred_at`, `decision`, `method`, `url_host`, `url_path`, `reason_code`, `safe_request_hash`, `denial_count`, `actor_kind`, and `signature`; optional `source_instance_hash`, `actor_ref_hash`, context-approved `workspace_id`, context-approved `project_id`, `probe_kind`, `url_path_hash`, `distinct_host_count`, `distinct_path_count`, and `distinct_actor_count`; and no unknown fields.
- **FR-007**: System MUST persist only lowercased URL host and parsed pathname, MUST exclude raw/full URL fields, scheme, userinfo, queries, fragments, CR/LF, blank host/path, and secret-like path values from accepted evidence, and MUST represent request identity through safe hashes or bounded counts only.
- **FR-008**: System MUST reject payloads containing raw/full URL fields, raw headers, raw bodies, cookies, Authorization values, API keys, query secrets, provider payloads, raw actor identifiers, user IDs, emails, or full CrabTrap audit rows.
...
- **FR-010**: System MUST reject malformed, unsigned, stale, replayed, oversized, unsafe, or unsupported payloads without writing activity evidence and MUST return a bounded failure code from the closed `crabtrap_intake_failure_code.v1` vocabulary; SPEC-011-supported methods are `GET`, `HEAD`, `POST`, `PUT`, `PATCH`, `DELETE`, and `OPTIONS` only, with `CONNECT`/`TRACE` deferred.
- **FR-011**: System MUST write exactly one existing-schema activity entry with `activities.type='security_intrusion_detected'` for each accepted unique denial summary, landing as `entity_type='workspace'`, `entity_id=workspace_id`, and `workspace_id=workspace_id` for approved workspace context or the real facility workspace row when no approved context exists.
- **FR-012**: System MUST keep activity data and diagnostics bounded to approved summary fields, safe hashes, bounded counts, and closed failure codes, and MUST never persist raw CrabTrap audit content, raw URLs, headers, bodies, cookies, tokens, query secrets, provider payloads, secret values, raw actor identifiers, raw secret hashes, or matched secret substrings.
...
- **CrabTrap Denial Summary**: A Paddock-owned signed fixture that safely and boundedly represents a denied proxy event through the flat `crabtrap_denial_summary.v1` schema. Key attributes are source, event id, signed and occurred timestamps, required `actor_kind`, optional keyed actor/source hashes, `decision="deny"`, supported method, lowercased URL host, parsed URL path, normalized reason code, safe request hash, bounded denial/count fields, and approved workspace/project scope.
...
- **Security Activity Evidence**: Existing activity-stream evidence with `activities.type='security_intrusion_detected'` and bounded data derived only from accepted denial summaries.
- **Replay Identity**: A uniqueness key derived from bounded event identity as `sha256:<64hex>` over `source + "\0" + event_id + "\0" + occurred_at`, persisted only as adapter-derived `data.replay_key_hash` in accepted activity evidence to prevent duplicate evidence without adding schema.
...
- Workspace/project scope is included only when the adapter context supplies approved bounded scope; otherwise accepted evidence uses the real facility workspace row without task or project mutation.
- Validated `project_id` is stored only in bounded activity `data`; the activity entity remains the workspace/facility landing row for this helper-only slice.
