...
**Strict Scope**: Add `src/lib/task-stage-attempts.ts`, `src/components/panels/task-stage-attempts-section.tsx`, and focused tests for those modules to `tsconfig.spec-strict.json` and `eslint.config.mjs`. Keep `src/app/api/tasks/[id]/stage-attempts/route.ts` and `src/components/panels/task-board-panel.tsx` under focused ESLint, route/component tests, and Playwright coverage rather than `tsconfig.spec-strict.json`: those integration wrappers import existing auth/workspace/task-detail modules whose legacy strict debt is outside SPEC-013A. The strict project still covers the new read model, API contract tests, component, and component tests directly.
