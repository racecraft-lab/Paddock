...
**Strict Scope**: Add these new TS files to `tsconfig.spec-strict.json` and `eslint.config.mjs`: `src/lib/task-claim-reconciliation.ts`, `src/app/api/tasks/[id]/claim-reconciliation/route.ts`, `src/lib/__tests__/migrations-M78-task-stage-claims.test.ts`, `src/lib/__tests__/task-claim-reconciliation-fixtures.ts`, `src/lib/__tests__/task-claim-reconciliation.test.ts`, `src/lib/__tests__/task-dispatch-claim-reconciliation.test.ts`, and `src/lib/__tests__/task-claim-reconciliation-route.test.ts`.
