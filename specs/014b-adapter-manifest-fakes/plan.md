...
**Strict Scope**: Add strict-compatible SPEC-014B modules and tests to `tsconfig.spec-strict.json` and `eslint.config.mjs`: `src/lib/harness-adapters/types.ts`, `src/lib/harness-adapters/fixtures.ts`, `src/lib/harness-adapters/evidence.ts`, `src/lib/harness-adapters/validation.ts`, `src/lib/harness-adapters/runtime-inventory.ts`, `src/components/agents/RuntimeInventoryEvidence.tsx`, `src/lib/harness-adapters/__tests__/validation.test.ts`, `src/lib/harness-adapters/__tests__/runtime-inventory.test.ts`, `src/components/agents/__tests__/RuntimeInventoryEvidence.test.tsx`, and `scripts/spec-014b/check-harness-adapter-scope.mjs`. The Next route, existing Agents panel, panel integration test, and Playwright scaffold remain covered by root `pnpm typecheck`, `pnpm lint`, focused Vitest, and Playwright because adding those files to the strict mini-project pulls legacy app-router/auth/database graphs outside SPEC-014B's bounded strict-scope slice.
