...
describe('SQLite busy_timeout pragma', () => {
  beforeEach(() => {
    vi.resetModules()
    delete process.env.PADDOCK_SQLITE_JOURNAL_MODE
  ...
  it('allows DELETE journal mode for Docker-mounted e2e databases', async () => {
    process.env.PADDOCK_SQLITE_JOURNAL_MODE = 'DELETE'
    const { getDatabase } = await import('@/lib/db')
    getDatabase()

    const pragmaCalls = mockPragma.mock.calls.map((c) => c[0] as string)
    expect(pragmaCalls).toContain('journal_mode = DELETE')
  })
