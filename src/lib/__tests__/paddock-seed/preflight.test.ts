...
import { runPaddockPreflight } from '@/lib/paddock-seed/preflight'
import { applyPaddockSeed } from '@/lib/paddock-seed/seed'
import { makeResidueSnapshotHash } from '@/lib/paddock-seed/evidence'
import {
  makePaddockSeedDb,
  paddockContractPath,
...
describe('paddock seed preflight', () => {
  it('blocks non-Paddock project, task, cron, OpenClaw, and FocusEngine residue without mutating rows', () => {
    ...
    const result = runPaddockPreflight(db, {
      contractPath: paddockContractPath(),
      operatorEvidencePath: operatorEvidenceFixturePath(),
    })
    const applyResult = applyPaddockSeed(db, {
      contractPath: paddockContractPath(),
      operatorEvidencePath: operatorEvidenceFixturePath(),
    })
    ...
    expect(db.prepare("SELECT COUNT(*) as count FROM workspaces WHERE slug = 'paddock'").get()).toEqual({ count: 0 })
  })
  ...
  it('redacts blocked output and references the cleanup runbook', () => {
    ...
    expect(result.ok).toBe(false)
    if (result.status !== 'blocked_preflight') throw new Error('expected blocked preflight')
    expect(result.cleanup_checklist).toBe('docs/runbooks/paddock-seed-predeploy.md')
  ...
  it('reports contract readiness failures with the requested preflight mode', () => {
    ...
    const result = runPaddockPreflight(db, { contractPath: missingContractPath }, 'apply')
