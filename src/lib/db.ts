...
const requestedJournalMode = (process.env.PADDOCK_SQLITE_JOURNAL_MODE || '').trim().toUpperCase()
const journalMode = requestedJournalMode === 'DELETE' ? 'DELETE' : 'WAL'

// Global database instance
let db: Database.Database | null = null;
const isBuildPhase = process.env.NEXT_PHASE === 'phase-production-build'
const isTestMode = process.env.PADDOCK_TEST_MODE === '1'
...
export function getDatabase(): Database.Database {
  if (!db) {
    ...
    // Docker-backed e2e opens the same bind-mounted database from host and container.
    // DELETE journaling avoids Docker Desktop WAL sidecar detachment in that harness.
...
type DbProcessGlobal = typeof globalThis & {
  __paddockDbCloseHandlers?: Set<() => void>
  __paddockDbProcessListenersRegistered?: boolean
}

const dbProcessGlobal = globalThis as DbProcessGlobal
const dbCloseHandlers = dbProcessGlobal.__paddockDbCloseHandlers ?? new Set<() => void>()
dbCloseHandlers.add(closeDatabase)
dbProcessGlobal.__paddockDbCloseHandlers = dbCloseHandlers

if (!dbProcessGlobal.__paddockDbProcessListenersRegistered) {
  ...
  dbProcessGlobal.__paddockDbProcessListenersRegistered = true
}
