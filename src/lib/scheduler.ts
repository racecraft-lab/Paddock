...
import { runGitHubSyncAutomationTick } from './github-sync-poller'
...
export function initScheduler() {
  ...
  tasks.set('github_sync_automation', {
...
async function tick() {
  ...
  for (const [id, task] of tasks) {
    ...
    const settingKey = id === 'auto_backup' ? 'general.auto_backup'
      ...
      : id === 'github_sync_automation' ? 'general.github_sync_automation'
      : 'general.agent_heartbeat'
    const defaultEnabled = id === 'agent_heartbeat' || id === 'webhook_retry' || id === 'claude_session_scan' || id === 'skill_sync' || id === 'local_agent_sync' || id === 'gateway_agent_sync' || id === 'task_dispatch' || id === 'aegis_review' || id === 'recurring_task_spawn' || id === 'stale_task_requeue' || id === 'github_sync_automation'
    ...
    try {
      const result = id === 'auto_backup' ? await runBackup()
        ...
        : id === 'github_sync_automation' ? await runGitHubSyncAutomationTick()
        : await runCleanup()
...
export function getSchedulerStatus() {
  ...
  for (const [id, task] of tasks) {
    ...
    if (task.lastResult) entry.lastResult = task.lastResult
    result.push(entry)
  }
...
export async function triggerTask(taskId: string): Promise<{ ok: boolean; message: string }> {
  ...
  if (taskId === 'github_sync_automation') return runGitHubSyncAutomationTick()
