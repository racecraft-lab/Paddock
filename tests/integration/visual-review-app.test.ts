...
describe('visual review app guidance', () => {
  ...
  it('posts a file-level inline PR review comment for the selected source file', async () => {
    const fetchMock = vi.fn(async (url: string, _init?: RequestInit) => {
      if (url.endsWith('/user')) {
        return {
          json: async () => ({ login: 'visual-reviewer' }),
          ok: true,
          status: 200,
        }
      }
    ...
    await vi.waitFor(() => expect(document.querySelector('.sync-badge')?.textContent).toContain('@visual-reviewer'))
  ...
  it('imports embedded merged PR state that was approved on the PR head SHA', async () => {
    ...
    expect(document.querySelector('.sync-status-line')?.textContent).toContain('@visual-reviewer: loaded 3 decision')
  })
